...
- **2026-04-11 wave 8/9 session 1** — Three wave-8/9 deliverables: **(1) consensus_reject_index** service `app/services/consensus_reject_index.{h,c}` — hash-keyed ring buffer for recent EV_CONSENSUS_REJECT events. Extended check_transaction/check_block wrappers to include `hash=<64hex>` in event payload. Bumped EVENT_PAYLOAD_SIZE 120→160. 14 test groups. **(2) chain rollback stress test** `lib/test/src/test_chain_rollback.c` — 20-block forward/backward walk via update_coins + disconnect_block, commitment consistency, XOR accumulator roundtrip, non-coinbase spend+undo path. 12 assertions. **(3) PHGR13 consensus fix** (WAVE_9 #1 priority) — both bugs from PHGR13_INVESTIGATION.md fixed: (a) rewrote `ppzksnark_vk_read` for libsnark's BINARY_OUTPUT + NO_PT_COMPRESSION format (is_zero flag + Montgomery LE limbs + text-encoded sparse_vector integers), VK now loads with ic_len=10, all G1 points verified on curve; (b) rewrote `bn_g2_decompress` with correct FE2IP decoding via u512_divmod_u256 (binary long division). 12 regression tests in `test_phgr13_fix.c`. Live node restarted and re-syncing — criterion #4 pending sync. `./test_zcl` green on every commit.
- **2026-04-12 wave 10 session 1** — Four items: **(1) Reorg safety test** `test_reorg_safety.c` — 24 checks: 50-block fork with disconnect/reconnect, UTXO integrity, CSR acceptance, recovery_policy, db_txn commit/rollback, non-coinbase spend undo, partial reorg, 5 rapid cycles. **(2) Consensus parity audit** — `tools/consensus_parity_audit.sh` + `CONSENSUS_PARITY.md`. C23 internal consistency verified (coins_best_block == chain tip at h=2,014,988). C++ cross-node comparison pending (node syncing). **(3) PHGR13 live verification CLOSED** — live node at h=2,014,988, past the h=2,014,948 barrier. Wave 9 #1 fully closed. **(4) Boot decomposition Phase A** — `block_index_loader.{h,c}` extracted from `boot_index.c`. 5 functions (577 lines), 8 tests. boot_index.c 1709→1132 lines. `./test_zcl` green on every commit.
