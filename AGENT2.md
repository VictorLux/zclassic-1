...
- **2026-04-11 wave 6 session 1** — **Wave 6 #6 (mempool_limits) done.** `app/services/mempool_limits.{h,c}`: env-tunable caps (`ZCL_MEMPOOL_MAX_BYTES` / `MAX_TXS` / `EXPIRY_SECONDS` / `MIN_RELAY_FEE_ZAT` / `LIMITS_TICK_SEC`), fee-per-byte eviction (cross-multiplied int64, no floats), expiry sweep with injectable clock, `EV_MEMPOOL_EVICT` / `EV_MEMPOOL_EXPIRE`, background pthread running periodic expire+enforce, idempotent start/stop. Seam into `lib/validation/src/txmempool.c`: new `tx_mempool_collect_views()` read-only snapshot + new `tx_mempool_set_post_add_hook()` called unlocked after successful adds — keeps lib/validation free of any app/services dependency. 12 tests cover defaults, env overrides, no-op-under-caps, fee/byte ordering (count cap), byte-cap eviction, fee/byte vs raw fee, expiry sweep, empty-pool expire, min-relay gate, automatic post-add hook, lifecycle idempotency, stats counters. Boot wiring queued in `BOOT_QUEUE.md`. `./test_zcl` green. **Also: wave 6 addrman persistence robustness done.** `app/services/addrman_integrity.{h,c}` mirrors `block_index_integrity` — 48-byte ADIX sidecar (magic+version+body_size+sha3) written next to `peers.dat` after the existing atomic rename, verified on every load. On any hash / size / magic / version mismatch the load path quarantines both files (rename aside as `<name>.corrupt.<ts>`) and starts fresh — a corrupt peers.dat is safer discarded than deserialized because its contents directly steer outbound peer selection. Wired into `connman_save_addrman` + `connman_load_addrman` (added `fsync` before rename in the save path while I was there). 11 tests for round-trip, missing sidecar (first-run), hash mismatch, truncation, body grown past sidecar, bad magic, unsupported version, quarantine event emission, idempotent rewrite, verdict-name stability, body-missing. New `EV_ADDRMAN_CORRUPT` event. `./test_zcl` green.
