...
static int64_t count_legacy_contacts(sqlite3 *db)
{
    sqlite3_stmt *s = NULL;
    int64_t count = -1;
    if (!db)
...
{
    const unsigned char *txt = sqlite3_column_text(s, col);
...
{
    if (!p || !p->db || !legacy_db || !projection_count || !legacy_count ||
        !first_diff || first_diff_len == 0)
        return false;
    first_diff[0] = '\0';
    *projection_count = (int64_t)contacts_projection_count(p);
    *legacy_count = count_legacy_contacts(legacy_db);

    sqlite3_stmt *ps = NULL;
    sqlite3_stmt *ls = NULL;
    const char *sql =
        "SELECT address,name,last_used FROM contacts ORDER BY address ASC";
    if (sqlite3_prepare_v2(p->db, sql, -1, &ps, NULL) != SQLITE_OK ||
        sqlite3_prepare_v2(legacy_db, sql, -1, &ls, NULL) != SQLITE_OK) {
    ...
    for (;;) {
        ...
        if (prc != SQLITE_ROW || lrc != SQLITE_ROW) {
            const char *key = prc == SQLITE_ROW ? col_text(ps, 0)
                                                : col_text(ls, 0);
            snprintf(first_diff, first_diff_len, "%s", key);
            break;
        }
        const char *pa = col_text(ps, 0);
        const char *la = col_text(ls, 0);
        int cmp = strcmp(pa, la);
        if (cmp != 0) {
            snprintf(first_diff, first_diff_len, "%s", cmp < 0 ? pa : la);
            break;
        }
        if (strcmp(col_text(ps, 1), col_text(ls, 1)) != 0 ||
            sqlite3_column_int64(ps, 2) != sqlite3_column_int64(ls, 2)) {
            snprintf(first_diff, first_diff_len, "%s", pa);
            break;
        }
    }
    sqlite3_finalize(ps);
