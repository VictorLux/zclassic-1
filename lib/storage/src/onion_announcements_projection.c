...
static int64_t count_legacy_onion_announcements(sqlite3 *db)
{
    sqlite3_stmt *s = NULL;
    int64_t count = -1;
    if (!db)
                                      ...
                                      size_t first_diff_len)
{
    if (!p || !p->db || !legacy_db || !projection_count || !legacy_count ||
        ...
        return false;
    first_diff[0] = '\0';
    *projection_count = (int64_t)onion_ann_projection_count(p);
    *legacy_count = count_legacy_onion_announcements(legacy_db);
    ...
    for (;;) {
        ...
        if (sqlite3_column_int64(ps, 1) != sqlite3_column_int64(ls, 1) ||
            strcmp(col_text(ps, 2), col_text(ls, 2)) != 0) {
