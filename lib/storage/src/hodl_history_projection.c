...
static int64_t count_legacy_hodl_history(sqlite3 *db)
{
    sqlite3_stmt *s = NULL;
    int64_t count = -1;
    if (!db)
                                         ...
                                         size_t first_diff_len)
{
    if (!p || !p->db || !legacy_db || !projection_count || !legacy_count ||
        ...
        return false;
    first_diff[0] = '\0';
    *projection_count = (int64_t)hodl_history_projection_count(p);
    *legacy_count = count_legacy_hodl_history(legacy_db);
    ...
    for (;;) {
        ...
        int64_t lh = lrc == SQLITE_ROW ? sqlite3_column_int64(ls, 0) : 0;
        if (prc != SQLITE_ROW || lrc != SQLITE_ROW) {
            snprintf(first_diff, first_diff_len, "%lld",
                     (long long)(prc == SQLITE_ROW ? ph : lh));
            break;
        }
        if (ph != lh) {
            snprintf(first_diff, first_diff_len, "%lld",
                     (long long)(ph < lh ? ph : lh));
            break;
        }
        if (sqlite3_column_int64(ps, 1) != sqlite3_column_int64(ls, 1) ||
            sqlite3_column_int64(ps, 2) != sqlite3_column_int64(ls, 2) ||
            sqlite3_column_int64(ps, 3) != sqlite3_column_int64(ls, 3) ||
            sqlite3_column_double(ps, 4) != sqlite3_column_double(ls, 4)) {
            snprintf(first_diff, first_diff_len, "%lld", (long long)ph);
            break;
        }
    }
    sqlite3_finalize(ps);
