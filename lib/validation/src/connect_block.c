    ...
    for (size_t i = block->num_vtx; i-- > 0; ) {
        ...
        if (i > 0) {
            ...
            for (size_t j = tx->num_vin; j-- > 0; ) {
                 ...
                 * (set above); otherwise the entry already holds it.
                 * Best-effort; never gates the consensus restore. */
                {
                    uint32_t add_height = (undo->height > 0)
                    ...
                    update_coins_emit_utxo_add_projection(
                        ...
                        undo->txout.script_pub_key.data,
                        (uint32_t)undo->txout.script_pub_key.size);
                }
            }
        }
