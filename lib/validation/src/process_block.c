...
/* Mark pindex and all descendants as BLOCK_FAILED_CHILD. */
static void mark_descendants_invalid(struct main_state *ms,
                                      struct block_index *pindex)
{
    size_t iter = 0;
    ...
    while (block_map_next(&ms->map_block_index, &iter, NULL, &bi)) {
        if (!bi) continue;
        /* Walk up from bi to see if pindex is an ancestor */
        struct block_index *walk = bi;
        while (walk && walk->nHeight > pindex->nHeight)
            walk = walk->pprev;
        if (walk == pindex && bi != pindex)
                      ...
                      const char *datadir)
{
    ...
    /* If the block is on the active chain, disconnect back past it */
    struct block_index *tip = active_chain_tip(&ms->chain_active);
    if (tip) {
        /* Check if pindex is on the active chain */
        struct block_index *at_height = active_chain_at(&ms->chain_active,
                                                         pindex->nHeight);
        if (at_height == pindex) {
            /* Disconnect tip down to pindex's parent */
            while (active_chain_tip(&ms->chain_active) != pindex->pprev) {
                if (!disconnect_tip(state, ms, coins_tip, datadir)) {
                    printf("invalidateblock: disconnect_tip failed at %d\n",
                           active_chain_height(&ms->chain_active));
                    return false;
                }
            }
    ...
    /* Mark the block and all descendants as invalid */
    pindex->nStatus |= BLOCK_FAILED_VALID;
    mark_descendants_invalid(ms, pindex);

    printf("invalidateblock: marked block %d as invalid\n", pindex->nHeight);
    return true;
}

/* Clear BLOCK_FAILED_MASK from pindex and all descendants */
static void clear_failed_flags(struct main_state *ms,
                                struct block_index *pindex)
{
...
bool reconsider_block(struct validation_state *state,
                      ...
                      const char *datadir)
{
    clear_failed_flags(ms, pindex);

    printf("reconsiderblock: cleared failed flags from block %d "
           "and descendants\n", pindex->nHeight);

    /* Re-evaluate best chain — may trigger reorg */
    return activate_best_chain(state, ms, coins_tip, params, NULL, datadir);
}
