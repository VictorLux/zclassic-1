 ...
 *      largest legal stack element) is accepted and round-trips. (The
 *      521-byte over-limit reject is intentionally NOT tested; see the
 *      inline note — it would overrun a fixed buffer in script_get_op.)
...
{
    int failures = 0;
    TEST_CASE("script_interp edge: CScriptNum 4-byte limit, 520-byte push, "
              "MINIMALDATA, op-count 201 cap, multisig missing-dummy, "
              "standalone OP_ELSE") {
        ...
        /* ---- 2. 520-byte push -> accept (== MAX_SCRIPT_ELEMENT_SIZE) ----
         * The largest legal stack element. The push itself must not be
         * rejected; the residual top is the 520-byte (truthy) element.
         *
         * We deliberately do NOT test the 521-byte over-limit case here:
         * script_get_op() memcpy's the declared element length into the
         * evaluator's fixed MAX_SCRIPT_ELEMENT_SIZE push buffer BEFORE the
         * push_len > MAX_SCRIPT_ELEMENT_SIZE guard runs, so a 521-byte
         * element would overrun that stack buffer (a separate latent
         * defect, not the verdict-correctness this file pins). A test that
         * triggers UB is not deterministic and would fault under ASAN, so
         * the over-limit reject is left to a fix of script_get_op itself.
         * The 520-byte boundary is the safe, load-bearing assertion: the
         * largest legal element must round-trip. */
        {
