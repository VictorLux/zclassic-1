...
int test_domain_consensus_locktime(void)
{
    ...
    }
    {
        ...
        DCL_CHECK("expiry_height=500, height=500 expired",
                  domain_consensus_tx_is_expired(&tx, 500));
        DCL_CHECK("expiry_height=500, height=501 expired",
    ...
    /* ── is_expiring_soon_tx ───────────────────────────────── */
    {
        ...
        memset(tx.vin[0].prevout.hash.data, 0xAA, 32);
        tx.vin[0].prevout.n = 0;
        /* threshold = 3. At next_height=1007 → check@1010 → expired. */
        DCL_CHECK("expiring_soon at next_height+threshold == expiry",
