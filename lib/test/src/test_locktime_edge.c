...
int test_locktime_edge(void)
{
     ...
     * UINT32_MAX. A normal in-range height stays not-expired. */
    {
        ...
        /* A negative height whose uint32 cast == UINT32_MAX (i.e. -1) is
         * the only way a signed-int height reaches the max boundary. */
        DCLE_CHECK("expiry=UINT32_MAX expired exactly when cast==UINT32_MAX",
                   domain_consensus_tx_is_expired(&tx, -1));
        free(tx.vin);
    }
     ...
     * exact threshold-window boundary, then a coinbase (never expires). */
    {
        ...
        memset(tx.vin[0].prevout.hash.data, 0xAA, 32);
        tx.vin[0].prevout.n = 0;
        /* window = +3. next=997 → check@1000 == expiry → expiring soon. */
        DCLE_CHECK("expiring_soon exactly at next+3 == expiry_height",
