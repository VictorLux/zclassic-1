...
#define EXPECTED_TOTAL      66
#define EXPECTED_OPS        14  /* status, health, kpi, mempool*, mininginfo,
                                 * benchmark, dbstats, filemanifest, events,
                                 * rpc, tools_list, self_test, logtail */
#define EXPECTED_CHAIN      10
#define EXPECTED_NET         7
...
{
    int failures = 0;
    TEST("controllers: ops domain has 14 tools") {
        register_all();
...
{
    int failures = 0;
    TEST("controllers: net domain has 7 tools") {
        register_all();
