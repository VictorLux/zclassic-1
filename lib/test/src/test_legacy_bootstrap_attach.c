 ...
 *
 * Unit tests for legacy_bootstrap_attach (Wave S, S-4b).
 *
 ...
 *   - Missing legacy datadir → LEGACY_ATTACH_OUTCOME_LEGACY_NOT_FOUND, soft skip
...
#define LBA_CHECK(name, expr) do { \
    printf("legacy_bootstrap_attach: %s... ", (name)); \
    if ((expr)) printf("OK\n"); \
    else { printf("FAIL\n"); failures++; } \
} while (0)

static void lba_tmpdir(char *buf, size_t n, const char *tag)
{
    snprintf(buf, n, "./test-tmp/lba_%d_%s", (int)getpid(), tag);
}
...
{
    printf("\n=== legacy_bootstrap_attach tests ===\n");
    int failures = 0;

    blocker_module_init();

    /* ── 1. Outcome name mapping ─────────────────────────────────────── */
    LBA_CHECK("name(DID_IMPORT)",
              strcmp(legacy_attach_outcome_name(LEGACY_ATTACH_OUTCOME_DID_IMPORT),
                     "did_import") == 0);
    LBA_CHECK("name(NOOP_SAME_TIP)",
              strcmp(legacy_attach_outcome_name(LEGACY_ATTACH_OUTCOME_NOOP_SAME_TIP),
                     "noop_same_tip") == 0);
    LBA_CHECK("name(RECOVERED_FROM_CRASH)",
              strcmp(legacy_attach_outcome_name(
                         LEGACY_ATTACH_OUTCOME_RECOVERED_FROM_CRASH),
                     "recovered_from_crash") == 0);
    LBA_CHECK("name(REFUSED_HAS_STATE)",
              strcmp(legacy_attach_outcome_name(
                         LEGACY_ATTACH_OUTCOME_REFUSED_HAS_STATE),
                     "refused_has_state") == 0);
    LBA_CHECK("name(LEGACY_NOT_FOUND)",
              strcmp(legacy_attach_outcome_name(LEGACY_ATTACH_OUTCOME_LEGACY_NOT_FOUND),
                     "legacy_not_found") == 0);
    LBA_CHECK("name(FAILED)",
              strcmp(legacy_attach_outcome_name(LEGACY_ATTACH_OUTCOME_FAILED),
                     "failed") == 0);

    /* ── 2. Atomicity contract — BEGIN IMMEDIATE + writes + ROLLBACK ── */
    {
        char dir[256];
        lba_tmpdir(dir, sizeof(dir), "atomic");
        mkdir_p(dir);

        LBA_CHECK("open progress.kv", progress_store_open(dir));
        sqlite3 *db = progress_store_db();
        LBA_CHECK("db non-NULL", db != NULL);

        /* Simulate the legacy_bootstrap_attach finalization pattern: an
        ...
        sqlite3_finalize(st);
        LBA_CHECK("stamp all 3 stage cursors", stamps_ok);
        ...
        size_t got = 0;
        uint8_t out[16] = {0};
        LBA_CHECK("post-rollback: sentinel get returns true",
                  progress_meta_get(db, "import_in_progress",
                                    out, sizeof(out), &got, &found));
        LBA_CHECK("post-rollback: sentinel NOT present", !found);
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (prep_ok) {
                ...
                LBA_CHECK("post-rollback: cursor row absent",
                          rc == SQLITE_DONE);
            }
            sqlite3_finalize(q);
        }
        ...
        LBA_CHECK("BEGIN IMMEDIATE for commit-path",
                  sqlite3_exec(db, "BEGIN IMMEDIATE",
                               NULL, NULL, NULL) == SQLITE_OK);
        LBA_CHECK("set sentinel for commit-path",
                  progress_meta_set_in_tx(db, "import_in_progress",
                                          &one, 1));
        LBA_CHECK("set tip height for commit-path",
                  progress_meta_set_in_tx(db, "legacy_attach_tip_height",
                                          (int32_t[]){3120921}, 4));
        LBA_CHECK("delete sentinel in same tx",
        ...
        found = true; got = 0;
        LBA_CHECK("post-commit: sentinel was deleted in tx",
                  progress_meta_get(db, "import_in_progress",
                                    out, sizeof(out), &got, &found) &&
                  !found);
        int32_t h_out = 0;
        LBA_CHECK("post-commit: tip_height observable",
        ...
        LBA_CHECK("reopen after commit", progress_store_open(dir));
        h_out = 0; got = 0; found = false;
        LBA_CHECK("tip_height survives reopen",
    ...
    /* ── 3a. Drift gate — LEGACY_ATTACH_STAGES_TO_STAMP exactly matches expected.
    ...
    {
        static const char *const EXPECTED_ATTACH_STAGES[] = {
            "header_admit",
            "validate_headers",
            "body_fetch",
        };
        size_t expected_n = sizeof(EXPECTED_ATTACH_STAGES) /
                            sizeof(EXPECTED_ATTACH_STAGES[0]);
        LBA_CHECK("stages count matches expected",
                  legacy_attach_stages_to_stamp_count() == expected_n);
        for (size_t i = 0; i < expected_n; i++) {
            char label[96];
            snprintf(label, sizeof(label),
                     "stage[%zu] is '%s'", i, EXPECTED_ATTACH_STAGES[i]);
            const char *got = legacy_attach_stages_to_stamp_at(i);
            LBA_CHECK(label, got && strcmp(got, EXPECTED_ATTACH_STAGES[i]) == 0);
        }
        LBA_CHECK("stages_at(count) returns NULL",
                  legacy_attach_stages_to_stamp_at(expected_n) == NULL);
        LBA_CHECK("stages_at(big) returns NULL",
                  legacy_attach_stages_to_stamp_at(99) == NULL);
    }
    ...
    {
        char dir[256];
        lba_tmpdir(dir, sizeof(dir), "rewind");
        mkdir_p(dir);

        LBA_CHECK("open progress.kv (rewind)", progress_store_open(dir));
        sqlite3 *db = progress_store_db();

        /* First stamp — fresh row, write happens. */
        bool was_write = false;
        LBA_CHECK("first stamp at 100 succeeds",
                  legacy_attach_stamp_one_for_test(db, "header_admit",
                                         100, &was_write));
        LBA_CHECK("first stamp reports was_write=true", was_write);
        ...
        sqlite3_finalize(q);
        LBA_CHECK("cursor stored as 100", cursor_after_first == 100);

        /* Forward stamp at 200 — write happens, cursor moves. */
        was_write = false;
        LBA_CHECK("forward stamp at 200 succeeds",
                  legacy_attach_stamp_one_for_test(db, "header_admit",
                                         200, &was_write));
        LBA_CHECK("forward stamp reports was_write=true", was_write);

        /* Backward stamp at 50 — anti-rewind kicks in: success, no write. */
        was_write = true;  /* deliberately wrong sentinel */
        LBA_CHECK("backward stamp at 50 succeeds (no-op)",
                  legacy_attach_stamp_one_for_test(db, "header_admit",
                                         50, &was_write));
        LBA_CHECK("backward stamp reports was_write=false", !was_write);
        ...
        int64_t cursor_after_rewind =
            rc == SQLITE_ROW ? sqlite3_column_int64(q, 0) : -1;
        sqlite3_finalize(q);
        LBA_CHECK("cursor preserved at 200 after backward stamp",
                  cursor_after_rewind == 200);

        /* Equal stamp at 200 — anti-rewind treats it as no-op. */
        was_write = true;
        LBA_CHECK("equal stamp at 200 succeeds (no-op)",
                  legacy_attach_stamp_one_for_test(db, "header_admit",
                                         200, &was_write));
        LBA_CHECK("equal stamp reports was_write=false", !was_write);

        /* Forward stamp at 5_000_000 — large jumps work. */
        was_write = false;
        LBA_CHECK("large forward stamp succeeds",
                  legacy_attach_stamp_one_for_test(db, "header_admit",
                                         5000000, &was_write));
        LBA_CHECK("large forward stamp reports was_write=true", was_write);

        /* Stamping a different stage starts fresh at the proposed value. */
        was_write = false;
        LBA_CHECK("fresh stage stamp at 1 succeeds",
                  legacy_attach_stamp_one_for_test(db, "validate_headers",
                                         1, &was_write));
        LBA_CHECK("fresh stage stamp reports was_write=true", was_write);
     ...
     * `make deploy`. */
    {
        ...
        LBA_CHECK("NULL our_datadir rejected",
                  !legacy_bootstrap_import_blocking(&null_our, &r));
        LBA_CHECK("NULL legacy_datadir rejected",
                  !legacy_bootstrap_import_blocking(&null_legacy, &r));
    }

    printf("legacy_bootstrap_attach: %d failures\n", failures);
    return failures;
}
