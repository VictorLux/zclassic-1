...
int test_slp(void)
{
    ...
    printf("SLP GENESIS build/parse roundtrip... ");
    {
        uint8_t script[512];
        size_t len = slp_build_genesis(script, sizeof(script),
            "ZTEST", "ZClassic Test Token",
            "https://zclassic.org", NULL,
            8, 0, 1000000000000ULL);

        struct slp_message msg;
        bool ok = slp_parse(script, len, &msg);
        ok = ok && (msg.type == SLP_TX_GENESIS);
        ok = ok && (strcmp(msg.ticker, "ZTEST") == 0);
        ok = ok && (strcmp(msg.name, "ZClassic Test Token") == 0);
        ok = ok && (msg.decimals == 8);
        ok = ok && (msg.initial_quantity == 1000000000000ULL);
        ok = ok && (msg.mint_baton_vout == 0);
        if (ok) printf("OK (%zu bytes)\n", len);
        else { printf("FAIL\n"); failures++; }
    }

    printf("SLP SEND build/parse roundtrip... ");
    {
        struct uint256 token_id;
        memset(token_id.data, 0xAB, 32);
        uint64_t amounts[] = {5000, 3000, 2000};

        uint8_t script[256];
        size_t len = slp_build_send(script, sizeof(script),
            &token_id, amounts, 3);
        ...
        ok = ok && (msg.output_quantities[0] == 5000);
        ok = ok && (msg.output_quantities[1] == 3000);
        ok = ok && (msg.output_quantities[2] == 2000);
        ok = ok && (memcmp(msg.token_id.data, token_id.data, 32) == 0);
        if (ok) printf("OK (%zu bytes)\n", len);
        else { printf("FAIL\n"); failures++; }
    }

    printf("SLP MINT build/parse roundtrip... ");
    {
        struct uint256 token_id;
        memset(token_id.data, 0xCD, 32);

        uint8_t script[256];
        size_t len = slp_build_mint(script, sizeof(script),
            &token_id, 2, 500000ULL);

        struct slp_message msg;
        bool ok = slp_parse(script, len, &msg);
        ok = ok && (msg.type == SLP_TX_MINT);
        ok = ok && (msg.additional_quantity == 500000ULL);
    ...
    printf("SLP reject non-SLP script... ");
    {
        uint8_t script[] = {0x6a, 0x04, 0xDE, 0xAD, 0xBE, 0xEF};
        struct slp_message msg;
        bool ok = !slp_parse(script, sizeof(script), &msg);
        if (ok) printf("OK\n");
        else { printf("FAIL (accepted non-SLP)\n"); failures++; }
    }
