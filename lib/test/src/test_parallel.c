...
#define TEST_LIST(X) \
    ...
    X(lag_slo) X(boot_phase) X(path_check) X(supervisor) X(condition_engine) X(blocker) \
    X(clock) X(rng) X(stage) X(mailbox) X(projection) X(progress_store) \
    X(header_admit_stage) \
