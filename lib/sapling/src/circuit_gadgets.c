...
static const uint8_t FR_MODULUS_MINUS_1[32] = {
...
/* Decompose an AllocatedNum into 255 bits with strict range proof.
 * Total constraints: 255 (allocations) + ~132 (kary_ands) + 1 (packing) = ~388.
 * Returns the allocated bit variable indices in bits_out (LE order, 255 bits). */
void gadget_into_bits_le_strict(struct constraint_system *cs,
                                  size_t allocated_var,
                                  size_t *bits_out,
                                  size_t *n_bits_out)
{
    struct fr one_val;
    ...
    /* Get the value bytes */
    struct fr val = cs->witness[allocated_var];
    uint8_t val_bytes[32];
    fr_to_bytes(val_bytes, &val);

    /* Get modulus-1 bytes (already in LE) */
    uint8_t mod_bytes[32];
    memcpy(mod_bytes, FR_MODULUS_MINUS_1, 32);

    /* Process bits from MSB to LSB of modulus-1.
     * modulus-1 is stored as LE bytes, so bit 0 = LSB of byte 0.
     * MSB iteration: start from bit 255 down to bit 0. */

    /* First, find the position of the highest set bit in modulus-1 */
    int first_one = -1;
    for (int i = 255; i >= 0; i--) {
        int byte_idx = i / 8;
        int bit_idx = i % 8;
        if ((mod_bytes[byte_idx] >> bit_idx) & 1) {
            first_one = i;
            break;
        }
    }
    /* For BLS12-381 Fr, first_one = 254 (bit 254 of r-1 is the highest set bit) */

    /* Allocate results array and tracking */
    size_t result[256];
    int result_count = 0;

    /* kary_and chain: track last_run (variable index) and current_run */
    size_t last_run = SIZE_MAX; /* SIZE_MAX = not set */
    size_t current_run[256];
    int current_run_len = 0;

    for (int i = 255; i >= 0; i--) {
        int byte_idx = i / 8;
        int bit_idx = i % 8;
        bool mod_bit = (mod_bytes[byte_idx] >> bit_idx) & 1;
        bool val_bit = (val_bytes[byte_idx] >> bit_idx) & 1;

        if (i > first_one) {
            /* Skip leading zeros — value bit must also be 0 */
            continue;
        }

        if (mod_bit) {
            /* In a run of 1s: allocate with boolean constraint */
            size_t bit = gadget_alloc_boolean(cs, val_bit);
            current_run[current_run_len++] = bit;
            result[result_count++] = bit;
        } else {
            /* End of run of 1s (if any) */
            if (current_run_len > 0) {
                /* Compute kary_and of current_run (+ last_run if exists) */
                size_t *and_input = current_run;
                int and_len = current_run_len;

                /* If last_run exists, add it to the input */
                size_t and_with_last[257];
                if (last_run != SIZE_MAX) {
                    memcpy(and_with_last, current_run, current_run_len * sizeof(size_t));
                    and_with_last[current_run_len] = last_run;
                    and_input = and_with_last;
                    and_len = current_run_len + 1;
                }

                /* kary_and: chain of AND gates */
                size_t cur = and_input[0];
                for (int j = 1; j < and_len; j++) {
                    struct fr av = cs->witness[cur];
                    struct fr bv = cs->witness[and_input[j]];
                    struct fr pv;
                    fr_mul(&pv, &av, &bv);
                    size_t p = cs_alloc_aux(cs, &pv);
                    gadget_mul(cs, cur, and_input[j], p);
                    cur = p;
                }
                last_run = cur;
                current_run_len = 0;
            ...
            /* alloc_conditionally: (1 - last_run) * bit = 0 */
            struct fr bit_val;
            if (val_bit) fr_one(&bit_val); else fr_zero(&bit_val);
            size_t bit = cs_alloc_aux(cs, &bit_val);

            /* Constraint: NOT(last_run) * bit = 0 */
            {
                struct linear_combination la, lb, lc;
                struct fr neg_one;
                fr_neg(&neg_one, &one_val);
                ...
                lc_add_term(&la, CS_ONE, &one_val);
                if (last_run != SIZE_MAX)
                    lc_add_term(&la, last_run, &neg_one);
                lc_init(&lb);
                lc_add_term(&lb, bit, &one_val);
                lc_init(&lc);
                cs_enforce(cs, &la, &lb, &lc);
                lc_free(&la); lc_free(&lb); lc_free(&lc);
            }
    ...
    /* Packing constraint: sum(result[i] * 2^bit_position[i]) = allocated_var
     * result is in MSB-first order, we need to reverse for LE packing */
    {
        struct linear_combination la, lb, lc;
        lc_init(&la);
        struct fr coeff;
        fr_one(&coeff);
        /* result is MSB-first, reverse to get LE */
        for (int i = result_count - 1; i >= 0; i--) {
            lc_add_term(&la, result[i], &coeff);
            fr_add(&coeff, &coeff, &coeff);
        }
        struct fr neg_one;
        fr_neg(&neg_one, &one_val);
        lc_add_term(&la, allocated_var, &neg_one);

        /* Enforce: 0 * 0 = (sum - value) → sum = value */
        lc_init(&lb);
        lc_init(&lc);
        lc_add_term(&lc, CS_ONE, &la.terms[0].coeff); /* hack: copy la into lc */
        /* Actually need the full la as the C side of 0*0=C */
        lc_free(&lb); lc_free(&lc);

        /* Better: lc = sum - value, enforce lc * 1 = 0... no.
         * Bellman uses: enforce(|lc| lc, |lc| lc, |_| lc_sum_minus_val)
         * which is: 0 * 0 = (sum - val). This is valid R1CS. */
        lc_init(&lb); /* empty = 0 */
        struct linear_combination lc2;
        lc_init(&lc2); /* empty = 0 */
        cs_enforce(cs, &lb, &lc2, &la);
        lc_free(&la); lc_free(&lb); lc_free(&lc2);
    }

    /* Output bits in LE order (reversed from result) */
    *n_bits_out = (size_t)result_count;
    for (int i = 0; i < result_count; i++) {
        bits_out[i] = result[result_count - 1 - i];
    }
}

/* ── field_into_boolean_vec_le (simple, no packing) ────────────── */

void gadget_field_into_boolean_vec_le(struct constraint_system *cs,
                                        const struct fr *value,
                                        size_t *bits_out,
                                        size_t n_bits)
{
