 ...
 *
 * Pure C23 replacements for librustzcash FFI.
...
void librustzcash_sapling_verification_ctx_free(void *ctx) { free(ctx); }
...
struct librustzcash_proving_ctx {
...
{
    return calloc(1, sizeof(struct librustzcash_proving_ctx));
}

void librustzcash_sapling_proving_ctx_free(void *ctx) { free(ctx); }
    ...
    unsigned char *zkproof)
{
    ...
    struct librustzcash_proving_ctx *pctx = ctx;
    if (pctx) {
        struct fs rcv_fs;
...
{
    (void)value_balance;
    const struct librustzcash_proving_ctx *pctx = ctx;
