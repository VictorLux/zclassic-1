...
|---|---|---|
| **CRITICAL** | 27 / 33 | **P24.18** (connect_tip silent-fail + Sapling end-height), **P24.19** (clean-shutdown + fast-boot, eliminates 13-min rebuild cycle) |
| **HIGH** | 35 / 59 | P12.3, P12.3.1, P13.2, P13.4, P13.6, P13.7, P14.4, P14.5, P14.15, P14.16, P15.1, P15.2, P15.4, P15.6, P16.1, P16.2, P17.1, P17.2, P17.4, P17.5, P17.6, P18.1, P19.1, P24.2, P24.3, P24.4, P24.5, P24.7, P24.8, P24.10, **P24.15**, **P11.8**, **P24.20** (error propagation + observability), **P24.21** (UTXO anchor roll-forward) |
...
**Owner state (2026-04-22 01:05 — P13.1 + P11.5 LANDED, P13.1 deploying, Agent-2 rotates to P24.18):**
- **Agent-2 NOW: P24.18** (connect_tip silent-fail + Sapling end-height — the stall fix). **P13.1 landed 96c8d32c6** — addnode-drain fallback prioritizes `-addnode` list over addrman when tried bucket is exhausted; test `test_connman_addnode_fallback.c` green. 76-line RED from Agent-2's prior session preserved via `wip/agent-2-p13.1-red` (7140999a8) and fully incorporated into GREEN merge. Coordinator deploying to live node now — 13-minute Sapling rebuild cycle expected (P24.19 will eliminate it next). **Remaining queue IN ORDER:** P24.18 → P24.19 → P24.20 → P24.21. Full details in [`AGENT-2.md`](AGENT-2.md).
- **Agent-3 NOW: P11.8** — parity diff gate (MVP #8, paired with Agent-2's P12.3 parity). **P11.5 landed ca3009006** (GREEN "persist store order status" + RED c704fa0e2 from prior push). Post-P11.8 lane: P15.4/P15.5 → P17 testing lead → P18.4 crypto perf → P20 dev-MCP → P21.7/P21.8 test split → P22.4 spec corpus → P23.7 scaffold_test_from_row → **P24.2 / P24.4**. Full checklist in [`AGENT-3.md`](AGENT-3.md).
- **Coordinator (Rhett):** P13.1 deploy triggered another 13-min Sapling rebuild (known cost, eliminated by P24.19). Once RPC is back up: canary P13.1 (should see 8+ outbound peers from `-addnode` list), then Agent-2 starts P24.18 which is the actual stall fix. **CRITICAL now = 27/33, 2 open** (P24.18, P24.19). HIGH 33/59 → 35/59 (P13.1 + P11.5). **Expected post-P24.18 live canary:** height advances from 3,078,014 toward 3,086,200+.
