...
| **MED** | 24 / 38 | **63%** | P7.10, P8.4, P8.6, P8.8, P9.6–P9.9, P12.4–P12.7, **P13.3** (sync controller noise), **P13.5** (addrman lookup) |
| **LOW** | 2 / 4 | **50%** | P9.10, P12.8 |
| (P0 baseline) | 4 / 4 | **100%** | — |

**Open by owner (2026-04-19 late night, post-reset):**
- **Agent-2 (0 rows — waiting on Rhett's live canary + CI investigation):** P10.1.4 fix landed `ac782fef5`. P8.7 num_chunks overflow guard landed opportunistically under the NEXT pre-authorization (`db3054901`) while the canary is still pending. `make test` remains green modulo the 2 pre-existing flaky lint-gate cases. Remaining P8 MEDs (P8.4/P8.6/P8.8) + P7.10 follow-up are ready to unblock once the live canary reports RSS plateau. **New out-of-scope finding:** `make ci` bus-errors in `test_cookie_rotation` under `ulimit -s unlimited` (reproduces on a clean `git stash` of any pending work, so it's independent of P10.1.4 + P8.7). Flagged for Rhett as a separate investigation — likely a stack-allocation / libevent interaction that emerged since the P10.1.4 push.
...
**Top remaining risks:** until P10.1 closes, the entire dev plan blocks. After P10.1: triage which P9 audit findings actually need fixes vs. which are theoretical, drain the deferred P8 MEDs (3 rows — P8.4/P8.6/P8.8; P8.7 already landed), then file Phase 2 work (continuous fuzzing in CI, consensus-parity diff service, sapling-tree checkpoint, structured logs).
...
- P8.7 (`db3054901`) landed opportunistically under the NEXT queue
  pre-authorization — the change is self-contained and doesn't touch
  the chain-stall path. Remaining P8 MEDs (P8.4, P8.6, P8.8) and the
  P7.10 follow-up stay **deferred** until P10.1 closes. Not cancelled
  — parked.
