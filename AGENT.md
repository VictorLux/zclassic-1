...
|---|---|---|
| **CRITICAL** | 27 / 32 | P13.1, **P24.18** (block stall after unclean-shutdown boot) |
| **HIGH** | 33 / 57 | P12.3, P12.3.1, P13.2, P13.4, P13.6, P13.7, P14.4, P14.5, P14.15, P14.16, P15.1, P15.2, P15.4, P15.6, P16.1, P16.2, P17.1, P17.2, P17.4, P17.5, P17.6, P18.1, P19.1, P24.2, P24.3, P24.4, P24.5, P24.7, P24.8, P24.10, **P24.15**, **P11.5**, **P11.8** |
...
**Owner state (2026-04-21 21:15 — P24.14 + P11.4 LANDED, both agents rotating):**
- **Agent-2 NOW: P13.1** HIGH (last CRITICAL dep). **P24.14 landed 9c1794086** — single squashed commit: new `app/controllers/src/rpc_chainstate_guard.{c,h}` chokepoint + `rpc_require_chainstate_lookup_ready()` guards on 16 callsites across 5 controllers (transaction, chain_inspect, wallet_diagnostic, wallet_rescan, repair, wallet) + `lib/test/src/test_rpc_safety.c` (198 lines) + `lib/test/fixtures/coins_lookup_guard_fixture.c` + `tools/scripts/check_coins_lookup_nullcheck.sh` lint. `./test_zcl` green. Side branch `codex/p24-14-chainstate-guard` also pushed by Agent-2. Next: **P13.1** → P14 drain (P14.4, P14.5, P14.15, P14.16) → P13/P12/P7/P8 drain → P15-P23 → rest of P24 wave. Full checklist in [`AGENT-2.md`](AGENT-2.md).
- **Agent-3 NOW: P11.5** HIGH — store e2e CI gate (MVP #5). **P11.4 landed** (5bb110b18 RED + b2ea2e153 GREEN + 12a02db1b coverage fix — rebased + pushed by coordinator since Agent-3's original base (27dd97e0c) pre-dated Agent-2's P24.14 merge and conflicted on Makefile + AGENT.md + test_make_lint_gates.c; HEAD versions preserved for all three). Shielded-payment end-to-end gate now runnable via `make test-shielded-payment` (checks `~/.zcash-params` first, runs with `ZCL_STRESS_TESTS=1`, exercises wallet-seeded transparent UTXO → z_sendmany → mempool admission → Sapling note decryption). Post-P11.5/P11.8 lane: P15.4/P15.5 → P17 testing lead → P18.4 crypto perf → P20 dev-MCP → P21.7/P21.8 test split → P22.4 spec corpus → P23.7 scaffold_test_from_row → **P24.2 / P24.4**. Full checklist in [`AGENT-3.md`](AGENT-3.md).
- **Coordinator (Rhett):** **P24.11 deployed live 2026-04-21 20:25.** P24.14 pending deploy — need `make deploy`, then canary the 5 affected RPC controllers (`zcl_getrawtransaction`, `zcl_walletaudit`, `zcl_listunspent`, `zcl_z_listunspent`, `zcl_rescanblockchain`) against the live node's inverted-tail state to confirm no SIGABRT. Recompute KPI (Robustness 3→6, Observability 5→7; expect +6 points). Open coord-lane: P24.16 (wallet.persistence false-unhealthy), P24.1 datadir sweep automation, P24.7 deploy pre-flight. **CRITICAL now = 27/31, only P13.1 + 3 others remain.**
...
| **P24.18** | **`connect_tip` silently fails after unclean-shutdown boot, block stall at anchor height.** Observed live 2026-04-21 20:25+ and 21:20+ (both post-deploy boots): tip pins at the UTXO anchor height (3,078,014) even though headers advance to 3,086,102 and `activate_best_chain` repeatedly tries `connect h=3078015`. Symptoms per flush cycle: `activate_best_chain: first connect h=3078015 last h=3078015 path_len=1` → `[coins] flush ok: max_height=3078014 utxos=1340591 tip_written=1 wrote=0 deleted=0` → `flush_coins: sapling_tree persist failed` → FSM loop. The `wrote=0 deleted=0` after a "successful" connect path means connect_tip aborted before adding any coins, yet the FSM logged `connecting->ready (behind_peers)` instead of propagating the failure. Also: zclassicd peer 127.0.0.1:8034 repeatedly feeds 160-header batches where the reported "tip" regresses (3,082,014 → 3,081,534 → 3,081,214 → 3,080,894) — possibly a downstream side-effect of the same failure. Hypotheses to verify: (1) Sapling tree root mismatch at h=3,078,015 because `sapling_tree_rebuild` finished at h=3,078,003 (the anchor, NOT the true tip) and the +12 blocks between anchor and tip_before_restart never re-added their commitments; (2) `node_db_state_set("sapling_tree", ...)` returning false but `connect_tip` ignoring the error; (3) block file at h=3,078,015 damaged. Distinct from P13.1 (peer selection) and P24.11/14 (RPC crashes). Pair with a RED test that simulates unclean-shutdown → reboot → advance past anchor, asserts tip progresses. Evidence: `~/.zclassic-c23/node.log` continuous `flush_coins: sapling_tree persist failed` stream from 2026-04-21 20:25 onwards. | `lib/validation/src/process_block.c:288-304` (flush_coins sapling branch), `lib/validation/src/connect_block.c`, `app/services/src/chain_restore_service.c` (sapling_tree_rebuild end height), `lib/test/src/test_unclean_shutdown_advance.c` (new) | **CRITICAL** | Agent-2 (after P13.1) |
