...
**Overall: 89 / 145 rows closed (61%) | KPIS estimate: ~44/100 (honest post-audit — see rubric below)**
...
| **P24.17** | **`node.log.old` sweep — 9.6 GB frozen log from 2026-04-10 rotting in datadir.** Observed 2026-04-21 05:15. `~/.zclassic-c23/node.log.old` is 9,629,225,872 bytes, last written 2026-04-10 19:41 — 11 days stale, never compressed, never archived. Second stale file: `node.log.old3` (13 MB, same era). These accumulate across restart events that don't trigger logrotate properly. Fix: extend P24.1 datadir-sweep boot hook to archive `node.log.old*` files to `~/zcl-backups/log-sweep-<ts>/` OR simply gzip-in-place if >100 MB and unchanged for 1 hour. Subsumed-into or paired-with P24.1. | `app/services/src/datadir_sweep_service.c` | LOW | Rhett (coord) — extends P24.1 |
