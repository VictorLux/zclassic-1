...
**Overall: 76 / 106 rows closed (72%) | SWRC ~77%** (P12.1 sapling checkpoint closed in 8fb7cb623; P14.1 + P14.2 landed in d67817dd2 as coupled RED→GREEN commit; P12.4 landed in d1fb2422e during the canary deploy; **P10.1.5 now CLOSED** — deploy of the coupled P14.1+P14.2+P12.1+P12.4 binary at 2026-04-19 18:18 UTC produced real canary signals: chain advanced 3,081,408→3,081,601, zero SAVEPOINT/BIP30 errors, RSS stable at 1.7 GB, 7 peers connected. See P10.1.5 row for the full observation set.)

| Tier | Closed / Total | % | Open rows |
|---|---|---|---|
| **CRITICAL** | 15 / 20 | **75%** | P8.10 (SUPERSEDED), P9.2 (deferred), **P14.3** (`zcl_syncdiag` SIGABRT — Agent-2 NOW), **P14.6** (BLOCK_FAILED_CHILD GC — promoted from HIGH/P12.2 — Agent-2), **P13.1** (single-peer sync — DEFERRED behind P14) |
...
- **Agent-2 (NOW: P14.3 `zcl_syncdiag` SIGABRT, then P14.6 BLOCK_FAILED_CHILD GC):** P14.1 + P14.2 landed in d67817dd2 as a coupled RED→GREEN commit — `coins_view_sqlite` now opens a dedicated sqlite3 handle per file-backed input, so `OP_Savepoint`'s `nVdbeWrite>0` guard can no longer trip the flush; `:memory:` keeps the shared-handle SAVEPOINT fallback for unit tests; the three-layer RED test exercises the real SQLite persistence path P10.1.3's null-backing variant couldn't. **Agent-2's NOW queue, in priority order:** (1) **P14.3** CRITICAL — fix the `zcl_syncdiag` SIGABRT so the coordinator can monitor the node without crashing it. (2) **P14.6** CRITICAL (was P12.2) — cap `BLOCK_FAILED_CHILD` propagation; the memory amplifier behind the repeated 6 GB cgroup OOMs. (3) **P14.4** HIGH — sync FSM debounce (279,135 flap events). (4) **P14.5** HIGH — `val.block_connected` must fire post-commit, not on receipt. (5) Coordinator re-runs live-node P10.1.5 canary on main (hard log signals only — per the earlier misread). **After P14 closes**, drain P13.1 + P12.2 (now P14.6) + P12.3 + P13.4 + P13.2 + P13.5 + P13.3 + P12.4 + P12.5 + P12.6 + P12.7 + P8.4 + P7.10 + P12.8 as previously queued.
...
**LIVE-NODE STATUS (2026-04-19, post-P14.1-deploy):** chain advancing (3,081,408→3,081,601 in first 15 min; legacy peer at h=3,083,628, ~2,027 block gap — closing). 7 peers connected. RSS 1.7 GB stable. Boot mode confirmed `coins_view_sqlite: dedicated connection (BEGIN IMMEDIATE)`. Zero SAVEPOINT failures, zero BIP30 trips, zero crashes since 18:18:47 UTC boot. P14 canary is GREEN; P10.1.5 closed. Next soak check at +24h.
