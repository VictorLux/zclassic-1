...
**Overall: 76 / 112 rows closed (68%) | SWRC ~73%** (+P14.11 + P14.12 filed after deploying Agent-2's P14.7 partial fix `b3f1903d4` surfaced deeper bugs. **The actual sync-blocker is P14.11**: block_index entries for 3,081,408-3,081,601 have `nBits=0x00000000` because the anchor/snapshot path populates nHeight/pprev but not nBits. Every incoming header for 3,081,602 gets rejected `bad-diffbits` (200+ rejects post-boot, 400 HEADER REJECT lines, peers disconnecting as `sync stalled`). **P14.12** is companion: `active_chain` array has only the tip entry — `getblockhash 3081600` returns "out of range" even though it's connected; anchor-restore didn't walk pprev back to populate `vtx[]`. Header tip (3,081,408) is now BEHIND chain tip (3,081,601) — reverse of normal. P14.8 + P14.10 + P14.7 sub-causes are still real but dwarfed by P14.11: fix P14.11 first so headers can flow; P14.8/P14.10/P14.7 then become relevant.)

| Tier | Closed / Total | % | Open rows |
|---|---|---|---|
| **CRITICAL** | 15 / 25 | **60%** | P8.10 (SUPERSEDED), P9.2 (deferred), **P14.11** (block_index nBits=0 on anchor-populated entries — Agent-2 NEW-NOW, actual sync blocker), **P14.12** (active_chain has only tip — ancestor heights out-of-range — Agent-2 NOW coupled), **P14.7** (post-canary FSM pin — still open, partial fix `b3f1903d4`), **P14.8** (block_already_seen), **P14.10** (activation skip dropped), **P14.3** (`zcl_syncdiag` SIGABRT), **P14.6** (BLOCK_FAILED_CHILD GC), **P13.1** (single-peer sync, deferred) |
...
- **Agent-2 (NOW: P14.7 chain pins at 3,081,601; NEXT: P14.3, P14.6):** P14.1 + P14.2 landed in d67817dd2 — storage flush is fixed. Canary deploy 18:18 UTC produced a clean first-3-min advance (3,081,408→3,081,601, zero SAVEPOINT/BIP30), **but chain then pinned.** New row P14.7 covers the FSM bug this exposed: blocks are received + queued but activation FSM exits `ready → connecting: new_block → connecting → ready: behind_peers` without calling activate_best_chain. See `AGENT-2.md` NOW section for full brief. **Agent-2's NOW queue, in priority order:** (1) **P14.7** CRITICAL — fix the activation FSM's `new_block` handler to actually call `activate_best_chain` before exiting `connecting`. (2) **P14.3** CRITICAL — `zcl_syncdiag` SIGABRT (demoted from NOW → NEXT because node isn't crash-looping now; only a concern if someone re-triggers it). (3) **P14.6** CRITICAL — cap `BLOCK_FAILED_CHILD` propagation (OOM amplifier; pair with P14.7 if it surfaces there). (4) **P14.4** HIGH — FSM debounce (may be naturally-cured by P14.7 since the flap is driven by `new_block`). (5) **P14.5** HIGH — `val.block_connected` post-commit emission. **After P14 closes**, drain P13.1 + P12.3 + P13.4 + P13.2 + P13.5 + P13.3 + P12.5 + P12.6 + P12.7 + P8.4 + P7.10 + P12.8.
...
- **Rhett (coordinator):** do NOT call `zcl_syncdiag` against the live node until P14.3 lands — every call crashes the node. `zcl_status` and `zcl_health` now reflect genuine state since P14.1 — cache/SQLite no longer diverge. 24h soak watch is the current coordinator task; if chain keeps advancing and no crashes, P14.1 is production-stable. Preferred live-node probes: `~/zclassic23/tools/zcl-rpc getblockcount`, `... getbestblockhash`, `... getpeerinfo`, and `tail -n 200 ~/.zclassic-c23/node.log | grep -E "connect_tip.*FAILED|SAVEPOINT.*failed"`.

**LIVE-NODE STATUS (2026-04-19, +35 min post-P14.1-deploy):** chain advanced 3,081,408 → 3,081,601 in first ~3 min, then pinned. NEW FINDING: headers at h=3,083,642, peers at h=3,083,641, but zero `connect_tip` attempts for height > 3,081,601 — blocks are being received and queued but the activation FSM exits `connecting → ready: behind_peers` without calling activate_best_chain. Memory climbed 1.7 GB → 3.3 GB in 16 min (94 MB/min download-queue accumulation). **No chain progress despite 6 active inbound MagicBean peers, zero SAVEPOINT failures, zero BIP30 errors.** This is P14.7 (new row) — a downstream FSM bug exposed by P14.1 unblocking the storage path. P14.1's storage fix holds; P14.7 blocks the NEXT layer. Agent-2 pivoted NOW from P14.3 → P14.7 since node is no longer crash-looping but still can't catch up to peers.
