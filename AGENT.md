...
| **P24.14** | **`rpc_getrawtransaction` → `coins_view_cache_get_coins` SEGV on inverted tail.** Symbol-resolved 2026-04-21 05:02 via `nm` on live crash backtrace. Crash site: `rpc_getrawtransaction+0x4AB` → `coins_view_cache_get_coins+0x1B3` → SIGSEGV → SIGABRT. Worker called `zcl_getrawtransaction` (or raw `getrawtransaction`) on a txid whose funding output is at a height in the 193-block P24.13 inverted tail (3,081,409..3,081,601 — present in `chain.blocks` but NOT in `block_index`). `coins_view_cache_get_coins` dereferences a NULL result from the lookup path. Distinct from P24.11 (`rpc_getsyncdiag` json_free UAF) — shares only that both are RPC→abort on post-P24.13 inverted state. Evidence: `~/.zclassic-c23/node.log` offset 219298. Fix shape: (a) NULL-check in `coins_view_cache_get_coins` return path, (b) graceful "tx not yet indexed" error in `rpc_getrawtransaction` when lookup returns NULL instead of asserting, (c) broader RPC-layer audit for post-P24.13-inversion assumptions (gettxout, listunspent, etc. share the coins_view_cache dependency). Pair with RED test that feeds an inverted-tail state + calls getrawtransaction, asserts graceful RPC error instead of SIGABRT. | `app/controllers/src/blockchain_controller.c` (rpc_getrawtransaction), `lib/storage/src/coins_view_cache.c` (get_coins NULL path), `lib/test/src/test_rpc_safety.c` (new) | **CRITICAL** | Agent-2 (after P24.13) |
