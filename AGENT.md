...
|---|---|---|
| **CRITICAL** | 27 / 33 | P13.1, **P24.18** (connect_tip silent-fail + Sapling end-height), **P24.19** (clean-shutdown + fast-boot, eliminates 13-min rebuild cycle) |
| **HIGH** | 33 / 59 | P12.3, P12.3.1, P13.2, P13.4, P13.6, P13.7, P14.4, P14.5, P14.15, P14.16, P15.1, P15.2, P15.4, P15.6, P16.1, P16.2, P17.1, P17.2, P17.4, P17.5, P17.6, P18.1, P19.1, P24.2, P24.3, P24.4, P24.5, P24.7, P24.8, P24.10, **P24.15**, **P11.5**, **P11.8**, **P24.20** (error propagation + observability), **P24.21** (UTXO anchor roll-forward) |
...
**Owner state (2026-04-21 22:30 — sync-robustness wave queued for Agent-2, Agent-3 on MVP drain):**
- **Agent-2 QUEUE (next 5 rows, IN ORDER — all CRITICAL-or-HIGH, all aim at "node is STICKY, STRONG, ROBUST"):**
  1. **P13.1** HIGH — single-peer regression (addrman tried-bucket exhaustion; only 2/10 addnodes connect).
  2. **P24.18** CRIT — `connect_tip` silent-fail + Sapling rebuild end-height bug. Live node is stuck at h=3,078,014 right now because of this. Three-part fix: rebuild to current tip, propagate persist errors, sticky recovery on mismatch.
  3. **P24.19** CRIT — clean-shutdown marker + WAL checkpoint on SIGTERM. Kills the 13-minute unclean-boot cycle so every deploy becomes cheap (≤30s restart). Without this fix, every deploy hits P24.18's cascade again.
...
- **Agent-3 NOW: P11.5** HIGH — store e2e CI gate (MVP #5). **P11.4 landed** (5bb110b18 RED + b2ea2e153 GREEN + 12a02db1b coverage fix — rebased + pushed by coordinator). Post-P11.5/P11.8 lane: P15.4/P15.5 → P17 testing lead → P18.4 crypto perf → P20 dev-MCP → P21.7/P21.8 test split → P22.4 spec corpus → P23.7 scaffold_test_from_row → **P24.2 / P24.4**. Full checklist in [`AGENT-3.md`](AGENT-3.md). Not assigned any sync-robustness work — those live in Agent-2's lane (app/controllers, lib/validation, lib/storage).
- **Coordinator (Rhett):** **P24.14 deployed live 2026-04-21 21:20, canary PASSED** (all 5 previously-UNSAFE tools return clean responses, zero SIGABRT). Chain tip currently pinned at h=3,078,014 pending P24.18 fix (zclassicd at h=3,086,113, gap 8,099). Not ramming through a manual bootstrap — delegating to Agent-2 so every future deploy benefits from the systemic fix, not just this one. **CRITICAL now = 27/33, 3 open (P13.1, P24.18, P24.19).** If Agent-2 delivers the five-row wave, KPI expected rise: Correctness 2→8, Robustness 6→9, Operability 5→9 (+14 points — rubric jumps 55 → ~75).
...
| **P24.18** | **`connect_tip` silent-fail + Sapling rebuild end-height bug — block stall at anchor.** Observed live 2026-04-21 20:25+ and 21:20+ (both post-deploy boots): tip pins at UTXO anchor (3,078,014) even though headers reach 3,086,102; `activate_best_chain` loops `connect h=3,078,015` with `wrote=0 deleted=0` (silent abort). `flush_coins: sapling_tree persist failed` recurring every flush cycle. Evidence: `~/.zclassic-c23/node.log` continuous stream from 2026-04-21 20:25 onwards. **Root-cause hypothesis (verify first):** `sapling_tree_rebuild` at `app/controllers/src/sync_controller.c:1506` uses `chain_tip = active_chain_height(chain)` as END; `active_chain` at boot is at h=3,078,003 (restored from UTXO anchor). So rebuild replays 476969..3,078,003 and stops. Then normal sync connects 3,078,004..3,078,014 incrementally (each block's output commitments append to the tree via `node_db_sync_connect_block_local` at sync_controller.c:487+). When we reach h=3,078,015, `node_db_state_set("sapling_tree", ...)` at process_block.c:300 returns false — possibly because the `node_db` handle is in a bad state after restart (WAL dirty / lock contention / a state-table invariant broken by the anchor rollback). **Fix plan (three-part):** (A) sync_controller.c:1506 — rebuild to the CURRENT active tip at every invocation; if tip advances during rebuild (possible if boot concurrency changes), loop until tip stops advancing. (B) process_block.c:293-304 — upgrade `persist failed` from fprintf-and-continue to: event emit (EV_SAPLING_PERSIST_FAIL), increment a failure counter in metrics, after N consecutive fails (N=3) return error from flush_coins so connect_tip sees it. (C) connect_tip — when flush error is sapling-persist, take the "sticky recovery" path: set `g_sapling_tree_rebuilding=true`, signal sync_controller to run `sapling_tree_rebuild` again with current tip; DON'T advance the FSM to `ready` until flush is clean. **RED test** `lib/test/src/test_unclean_shutdown_advance.c`: simulate unclean shutdown at h=N, rebuild ending at h=N-11, assert node advances past h=N without stalling. **Acceptance (live canary):** after deploy, node advances from 3,078,014 toward 3,086,113 within 5 min, zero `persist failed` log lines, `zcl_kpi` wallet.persistence + chain invariants stay green. | `app/controllers/src/sync_controller.c:1498-1730` (sapling_tree_rebuild), `lib/validation/src/process_block.c:288-304` (flush_coins), `lib/validation/src/connect_block.c` (connect_tip error path), `lib/test/src/test_unclean_shutdown_advance.c` (new) | **CRITICAL** | Agent-2 (after P13.1) |
