...
## Progress — last update 2026-04-19 (P10.1.1 `1243e1766` + P10.1.2 `5279752d1` + P10.1.3 RED `ae7caa1fe` + P10.1.4 fix `ac782fef5` + P11.3 cold-start CI + P11.7 kill-9 recovery CI land + P8.7 num_chunks overflow guard `8e5522a8b` + P8.6 token_key disambiguation `93936c5fb` both land opportunistically; `make test` back to green modulo pre-existing lint-gate flakes; `make ci` bus-errors in test_cookie_rotation pre-existing — flagged for Rhett; only P10.1.5 live-node canary remains — coordinator row; **CI-verified MRS bumps 1/8 → 3/8**)

**Overall: 70 / 100 rows closed (70%) | SWRC ~76%** (denominator bumped: +5 P13.1–P13.5 from post-deploy live evidence — chain advancing but with sync UX issues)
...
| **MED** | 25 / 38 | **66%** | P7.10, P8.4, P8.8, P9.6–P9.9, P12.4–P12.7, **P13.3** (sync controller noise), **P13.5** (addrman lookup) |
| **LOW** | 2 / 4 | **50%** | P9.10, P12.8 |
| (P0 baseline) | 4 / 4 | **100%** | — |

**Open by owner (2026-04-19 late night, post-reset):**
- **Agent-2 (0 rows — waiting on Rhett's live canary + CI investigation):** P10.1.4 fix landed `ac782fef5`. Two P8 MEDs landed opportunistically under the NEXT pre-authorization while the canary is still pending: P8.7 num_chunks overflow guard (`8e5522a8b`) + P8.6 token_key disambiguation (`93936c5fb`). `make test` remains green modulo the 2 pre-existing flaky lint-gate cases. Remaining P8 MEDs (P8.4/P8.8) + P7.10 follow-up are ready to unblock once the live canary reports RSS plateau. **Out-of-scope finding:** `make ci` bus-errors in `test_cookie_rotation` under `ulimit -s unlimited` (reproduces on a clean `git stash` of any pending work, so it's independent of P10.1.4/P8.6/P8.7). Flagged for Rhett as a separate investigation — likely a stack-allocation / libevent interaction that emerged since the P10.1.4 push.
- **Agent-3 (Priority 11 — MVP CI):** P10.1.2 review landed `879192ee2` (CONCUR_WITH_NOTES — Agent-2's P10.1.4 fix adopts the "unconditional fix" note). Sync-focused MVP CI track moving — P11.1 onion bootstrap landed `63f98909d`, P11.3 cold-start sync landed `ffd1112e4`, P11.7 kill -9 chaos recovery landed `8d3d3b23f` (CI-verified MRS 1/8 → 3/8). Queue: P11.4/5/6/8 remain as TBD MVP CI gates. P9 sapling-prover audit (`04247c19a`) remains **deferred** until P10.1.5 closes.
- **Rhett (1 row, NOW):** **P10.1.5 live-node verification.** `make deploy`; watch for `EV_BLOCK_CONNECTED h=3,081,408` within 120s; log RSS plateau over 24h; update the P10.1.5 row with `done` + canary observations.
...
**Top remaining risks:** until P10.1 closes, the entire dev plan blocks. After P10.1: triage which P9 audit findings actually need fixes vs. which are theoretical, drain the remaining P8 MEDs (2 rows — P8.4/P8.8; P8.6/P8.7 already landed), then file Phase 2 work (continuous fuzzing in CI, consensus-parity diff service, sapling-tree checkpoint, structured logs).
...
- P8.6 (`93936c5fb`) and P8.7 (`8e5522a8b`) landed opportunistically
  under the NEXT queue pre-authorization — both changes are
  self-contained and don't touch the chain-stall path. Remaining P8
  MEDs (P8.4, P8.8) and the P7.10 follow-up stay **deferred** until
  P10.1 closes. Not cancelled — parked.
