...
- **Agent-2 (0 rows — waiting on Rhett's live canary + CI investigation):** P10.1.4 fix landed `ac782fef5`. P8.7 num_chunks overflow guard landed opportunistically under the NEXT pre-authorization (`8e5522a8b`) while the canary is still pending. `make test` remains green modulo the 2 pre-existing flaky lint-gate cases. Remaining P8 MEDs (P8.4/P8.6/P8.8) + P7.10 follow-up are ready to unblock once the live canary reports RSS plateau. **New out-of-scope finding:** `make ci` bus-errors in `test_cookie_rotation` under `ulimit -s unlimited` (reproduces on a clean `git stash` of any pending work, so it's independent of P10.1.4 + P8.7). Flagged for Rhett as a separate investigation — likely a stack-allocation / libevent interaction that emerged since the P10.1.4 push.
...
- P8.7 (`8e5522a8b`) landed opportunistically under the NEXT queue
