...
## Progress — last update 2026-04-19 (P10.1.1 `1243e1766` + P10.1.2 `5279752d1` + P10.1.3 RED `ae7caa1fe` land; Agent-2 pivots to P10.1.4 minimal fix — replace `coins_map_erase` at `connect_block.c:639` with a DIRTY+pruned tombstone + post-disconnect invariant assertion)
...
- **Agent-2 (1 row, NOW):** **P10.1.4 — minimal fix + invariant assertion**. Replace `coins_map_erase(&view->cache_coins, &tx->hash)` at `lib/validation/src/connect_block.c:639` with a DIRTY+pruned tombstone via `coins_view_cache_modify` so the scratch flush propagates a DELETE signal to the backing store. Add a post-`disconnect_tip` debug assertion walking the disconnected block's txs to enforce the P10.1.2 invariant. Flips P10.1.3 (`ae7caa1fe`) from RED to GREEN. Two remaining P10.1 sub-rows after this: P10.1.4 fix → P10.1.5 live canary. **No hotfix shortcuts.** P8.4/P8.6/P8.7/P8.8 + P7.10 follow-up stay deferred until P10.1 closes.
