...
**Coordinator drive note (2026-04-24 05:10 UTC):** Vibepoint currently returns mixed QEDC state and SHAs that do not exist in this repository. For zclassic23, use local repo state + tracked mail + live-node RPC as authoritative until the MCP board is repaired. Ignore any P26/service-registry instructions unless matching rows are added to this `AGENT.md`.
...
- **Agent-2 NOW:** resume zclassic P24 wave at **P24.31** unless Rhett explicitly adds a newer zclassic row to this file. P24.28/P24.29/P24.30 are landed in local history (`a940dd5ba` + `0af03d99e`, `ec1867c95` + `069f9b8bd`, `10a8a8fec` + `b3c98d6d0`). Ignore P26/service-registry mail from the mixed Vibepoint board.
...
- **Coordinator:** do not edit worker checkouts. Drive via tracked mail + `AGENT.md` until the zclassic MCP board is repaired.
