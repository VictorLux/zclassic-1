...
|---|---|---|
| **CRITICAL** | 27 / 38 | **P24.28** (hot-loop RED test — Agent-2 NOW, pays back debt on coord commit ddd1fbeab), **P24.29** (self-heal scan fallback — real stall fix), **P24.18a** (repro + diagnostic), **P24.18b** (Sapling end-height fix), **P24.19** (clean-shutdown + fast-boot), **P24.24** (transactional chain advance) |
| **HIGH** | 35 / 73 | P12.3, P12.3.1, P13.2, P13.4, P13.6, P13.7, P14.4, P14.5, P14.15, P14.16, P15.1, P15.2, P15.4, P15.6, P16.1, P16.2, P17.1, P17.2, P17.4, P17.5, P17.6, P18.1, P19.1, P24.2, P24.3, P24.4, P24.5, P24.7, P24.8, P24.10, **P24.15**, **P11.8**, **P24.18c**, **P24.20**, **P24.21**, **P24.22**, **P24.23**, **P24.25**, **P24.26**, **P24.27**, **P24.30**, **P25.1-P25.6** (agent-coord MCP wave) |
...
**Owner state (2026-04-22 04:45 — LIVE-STALL ROOT CAUSE IDENTIFIED + coordinator-landed escape hatch):**
...
| **P24.29** | **Proper UTXO self-heal fallback: scan block files by height when tx_index misses.** The 2026-04-22 live stall exposed the failure mode: block 3,078,015 spent a coinbase output from h=3,077,892; self-heal in `process_block.c:1277` called `block_tree_db_read_tx_index(...)` and got "not in tx index" because LDB-fast-synced blocks never get indexed. Dead self-heal → 4,700-failure hot loop. **Fix plan:** when `block_tree_db_read_tx_index` misses, fall back to a height-hint-based scan: (1) the missing txid belongs to some block; if it's a COINBASE, the spending input's scriptSig encodes the producing-block height (BIP34); decode + read that block from disk; scan its txs; inject UTXO if match. (2) if not a coinbase (no height hint), fall back to a bounded-depth scan (default last 1000 blocks from tip) — cheap and covers the "recently-imported" window. (3) on successful recovery, also populate the tx_index entry so the next spend of the same tx is O(log N). RED test: inject a coinbase-spend-without-tx-index fixture, assert self-heal recovers + tx_index is populated afterward. | `lib/validation/src/process_block.c:1261-1340` (extend self-heal), `lib/storage/src/txdb.c` (tx_index write helper if not exposed), `lib/test/src/test_self_heal_scan_fallback.c` (new) | **CRITICAL** | Agent-2 (after P24.28) |
...
- Each agent Codex spawns its own `zclassic23 -mcp` subprocess; they share state via the filesystem (`mail/` + `AGENT.md` + git), not via shared process memory. This matches qedc-mcp's pattern — the server is a thin protocol handler; authoritative state is on disk.
...
| **P25.2** | **Dashboard + agent NOW extraction tools.** Add to agent_controller.c: (a) `zcl_coord_dashboard` — parses AGENT.md → returns markdown dashboard (rollup counts, open CRITICAL, current wave, both agents' NOW). (b) `zcl_coord_agent_now(agent=2\|3)` — extracts `## Current status — NOW = ...` from AGENT-<N>.md + any `⚡ ACTION LIST` block. (c) `zcl_coord_kpi` — parses the rubric table from AGENT.md + computes current score. Reuse `slurp_file` / line-parser helpers from qedc_mcp.c (Apache-licensed, cite in NOTICE). Tests use fixture AGENT.md snippets. | `tools/mcp/controllers/agent_controller.c` (+~250 lines), `lib/test/src/test_agent_controller_dashboard.c` (new), `lib/test/fixtures/agent_md_*.txt` (new) | HIGH | Agent-2 |
| **P25.3** | **Mailbox infrastructure — filesystem + git-backed messaging.** Directory layout: `mail/a<N>/from_<sender>_c<cycle>_<slug>.md` with YAML frontmatter (from/to/cycle/subject/urgency). Tools: `zcl_coord_mail_inbox(agent)`, `zcl_coord_mail_read(agent, filename)`, `zcl_coord_mail_send(to, from, subject, body, urgency?, cycle?)`, `zcl_coord_mail_archive(agent, filename)`. `mail_send` writes file + `git add + commit + push` with author `zcl-coord-mcp <coord@local>`. Safety: basename-only (no path traversal), agent range 2..3 + "coord". Port from qedc_mcp.c lines 275-450. | `tools/mcp/controllers/agent_controller.c` (+~400 lines), `mail/a2/.gitkeep`, `mail/a3/.gitkeep`, `mail/coord/.gitkeep`, `lib/test/src/test_agent_controller_mailbox.c` (new) | HIGH | Agent-2 |
...
| **P25.8** | **Migrate coordinator workflow + retire ACTION LIST sections.** Once P25.1-P25.7 land: switch my pattern from "edit AGENT-2.md ⚡ ACTION LIST + git push" to `zcl_coord_mail_send(to=2, subject="P24.18 plan", body=...)`. Agent-2 checks `zcl_coord_mail_inbox` on kickoff. AGENT-2.md becomes a thin lane-rules shell pointing at the MCP tools. Retire `## ⚡ ACTION LIST` blocks — they become mail bodies. | `AGENT.md`, `AGENT-2.md`, `AGENT-3.md` (trim), `docs/agent_mcp_setup.md` | MED | Coordinator (after P25.1-7 all land) |

**P25 parallelism:** sequential (each row extends the prior's agent_controller.c). Agent-3 untouched — stays on P11.8 / P24.27.

**Why this changes vs qedc-2 approach:** qedc-2 built a standalone binary because their node+coordination domains were distinct projects. zclassic23 already has a rich MCP surface (60+ node-ops tools), so adding a controller class is cheaper + preserves single-binary ethos + zero new service to run. We borrow qedc's tool shapes (send_mail, dashboard, etc.) but host them in-process.
