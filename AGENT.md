...
## Progress — last update 2026-04-20 (P9.5 landed ff25fc779 — pthread_once on lazy Sapling caches; live node still pinned per P14.11 signature — `bad-diffbits` on every incoming headers batch from every peer, header_height=3,081,408 behind chain_height=3,081,601, 12 inbound peers, 2.4 GB RSS stable at 78 min uptime — confirms anchor path left nBits=0 + active_chain ancestors missing.)

**Overall: 77 / 112 rows closed (69%) | SWRC ~74%** (P9.5 in with RED+GREEN tests; P14.11+P14.12 remain the actual sync blocker — block_index entries for 3,081,408-3,081,601 have `nBits=0x00000000` because the anchor/snapshot path populates nHeight/pprev but not nBits. Every incoming header for 3,081,602 gets rejected `bad-diffbits` continuously (live evidence via `zcl_logtail`: every peer's 160-header batch rejects with header[0] `bad-diffbits` + headers[1..] cascading `bad-prevblk`). **P14.12** is companion: `active_chain` array has only the tip entry — `getblockhash 3081600` returns "out of range" even though it's connected; anchor-restore didn't walk pprev back to populate `vtx[]`. Header tip (3,081,408) is now BEHIND chain tip (3,081,601) — reverse of normal. Fix P14.11 first so headers can flow; P14.8/P14.10/P14.7 sub-causes then become relevant.)
...
| **HIGH** | 30 / 37 | **81%** | P9.1, P9.3 (Agent-3 NOW), P9.4, **P12.2** (Agent-2 NEXT after P13.1), **P12.3** (Agent-2 NEXT), **P13.2** (header tip oscillation — Agent-2), **P13.4** (IBD throughput 5-8× slower than zclassicd — Agent-2) |
| **MED** | 26 / 38 | **68%** | P7.10, P8.4, P9.6–P9.9, P12.4–P12.7, **P13.3** (sync controller noise), **P13.5** (addrman lookup) |
| **LOW** | 2 / 4 | **50%** | P9.10, P12.8 |
| (P0 baseline) | 4 / 4 | **100%** | — |

**Open by owner (2026-04-20, post-P9.5):**
- **Agent-2 (NOW: P14.11 + P14.12 — the actual sync blocker):** Chain pinned at 3,081,601 because block_index entries for 3,081,408-3,081,601 have `nBits=0x00000000` (anchor/snapshot path populates nHeight/pprev but NOT nBits). Every incoming header rejects `bad-diffbits`; `active_chain.vtx[]` has only the tip. **Agent-2's NOW queue, in priority order:** (1) **P14.11** CRITICAL — fix anchor/snapshot acceptance to write nBits; boot-time block_index integrity assert. (2) **P14.12** CRITICAL — walk pprev back to rebuild `active_chain.vtx[]` post-anchor-restore. Land as a coupled pair since both originate from the same anchor-restore shortcut. (3) **P14.10** CRITICAL — deferred-activation queue for SKIP_ALREADY_RUNNING (cleanup after P14.11 closes; P14.8 already shipped makes this non-load-bearing but still worth landing). (4) **P14.3** CRITICAL — `zcl_syncdiag` SIGABRT (coordinator touch-trap). (5) **P14.6** CRITICAL — cap `BLOCK_FAILED_CHILD` propagation (OOM amplifier). (6) **P14.4** HIGH — FSM debounce. (7) **P14.5** HIGH — `val.block_connected` post-commit emission. **After P14 closes**, drain P13.1 → P12.3 → P13.4 → P13.2 → P13.5 → P13.3 → P12.5 → P12.6 → P12.7 → P8.4 → P7.10 → P12.8.
- **Agent-3 (NOW: P9.3 — `lc_add_term` / `cs_alloc_var` / `cs_enforce` OOM silent-drop):** P12.1 (sapling checkpoint) shipped 8fb7cb623; P9.5 (pthread_once on lazy Sapling caches) shipped ff25fc779. Next per NEXT queue is **P9.3** — return bool + propagate + LOG_FAIL on realloc failure at `groth16_prover.c:38-44, 91-103, 117-145`. Small, HIGH severity. Queue after: P9.4 → P9.1 → P9.2 → P9.6-10 → P11.4/5/6/8.
