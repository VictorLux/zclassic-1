...
## Progress — last update 2026-04-20 (P14.11 + P14.12 canary FAILED — `chain_restore_rebuild_active_chain` is O(N²) at live-scale block_map → boot hangs for hours; P14.13 FILED to Agent-2 as new CRITICAL NOW. P9.3 landed 86ebfc4b5; P9.5 landed ff25fc779.)

**Overall: 80 / 113 rows closed (71%) | SWRC ~77%** (P14.11+P14.12 shipped code but the live-node canary at 19:35 UTC revealed a new CRITICAL regression: the `chain_restore_rebuild_active_chain` residual-holes branch does `tip_h × map_size` = ~10 trillion ops when the anchor's `pprev` is NULL. Boot pinned at 100% CPU with no progress for >5 min before coordinator stopped the service. Test coverage at H=10 doesn't surface the pathology. Filed P14.13 as the new Agent-2 NOW row with option-1 bucketing fix + wall-clock RED test requirement. P14.11+P14.12 code still correct in principle — the integrity check and backfill both work — but the rebuild path must not degenerate at real chain depth.)

| Tier | Closed / Total | % | Open rows |
|---|---|---|---|
| **CRITICAL** | 17 / 26 | **65%** | P8.10 (SUPERSEDED), P9.2 (deferred), **P14.13** (rebuild O(N²), Agent-2 NOW after 19:35 canary fail), **P14.7** (post-canary FSM pin — still open, partial fix `b3f1903d4`), **P14.8** (block_already_seen), **P14.10** (activation skip dropped), **P14.3** (`zcl_syncdiag` SIGABRT), **P14.6** (BLOCK_FAILED_CHILD GC), **P13.1** (single-peer sync, deferred) |
...
**Open by owner (2026-04-20, post-P14.13 FILED):**
- **Agent-2 (NOW: P14.13 — `chain_restore_rebuild_active_chain` O(N²) → boot-hang fix):** P14.11+P14.12 shipped `5f04aef62` but the 19:35 UTC canary revealed the residual-holes branch does `tip_h × map_size` scans when the anchor's `pprev` is NULL (= always, since anchor is synthesized at UTXO layer). Boot pinned at 100% CPU, never opened RPC. Preferred fix: single-pass bucketing by height (O(M) time + 24 MB temp alloc). Acceptance: RED test asserts wall-clock <2s at N=100,000 synthetic depth; live-node boot reaches `[chain-integrity] post-restore check OK` within 30s of `Chain restore: anchor`; RPC up within 60s; zero `bad-diffbits` lines after boot. **Agent-2's remaining NOW queue, in priority order after P14.13:** (1) **P14.10** CRITICAL — deferred-activation queue for SKIP_ALREADY_RUNNING (cleanup). (2) **P14.3** CRITICAL — `zcl_syncdiag` SIGABRT (coordinator touch-trap). (3) **P14.6** CRITICAL — cap `BLOCK_FAILED_CHILD` propagation (OOM amplifier). (4) **P14.4** HIGH — FSM debounce. (5) **P14.5** HIGH — `val.block_connected` post-commit emission. **After P14 closes**, drain P13.1 → P12.3 → P13.4 → P13.2 → P13.5 → P13.3 → P12.5 → P12.6 → P12.7 → P8.4 → P7.10 → P12.8.
...
- **Rhett (coordinator):** **DO NOT call `zcl_syncdiag`** against the live node until P14.3 lands — every call crashes the node. `zcl_status` is safe. Post-P14.13 canary: after Agent-2 ships the fix, rebuild + `make deploy` + confirm boot reaches integrity-OK + RPC up within 60s + chain past 3,081,601.

**LIVE-NODE STATUS (2026-04-20 19:45 UTC):** service STOPPED by coordinator at 19:40 after 5-min boot hang on the `5f04aef62` binary. The fresh binary never opened RPC — `chain_restore_rebuild_active_chain` burns 100% CPU on the residual-holes branch. No data corruption (read-only scan). Node WAS healthy at 14:38 deploy on prior binary: chain pinned at 3,081,601 / header_height 3,081,408, 11 inbound peers, RSS 2.4 GB stable at 4h39m uptime, exact P14.11 `bad-diffbits` signature. Once P14.13 fix lands, the P14.11+P14.12 canary should finally close.
...
| **P14.13** | **`chain_restore_rebuild_active_chain` O(N²) at live scale — boot hangs.** Coordinator canary of `5f04aef62` at 19:35 UTC: fresh binary booted past `Chain restore: anchor at h=3081601 — syncing forward.` and then pinned at 100% CPU for >5 min with no log progress, no RPC listener. Root cause: anchor's `pprev` is NULL (anchor synthesized at UTXO layer when `Coins DB best block NOT found in block_index`); first loop in `chain_restore_rebuild_active_chain` fills only `chain[tip_h]`; second loop iterates `tip_h = 3,081,601` times, each doing a full `block_map_next` scan over `map_size = 3,337,146` entries = ~10 trillion ops (~28 h at cache-miss rate). Test coverage uses H=10 — pathology invisible. **Fix:** single-pass bucketing — scan block_map ONCE, bucket by height into a temp `block_index*[tip_h+1]` array (prefer BLOCK_HAVE_DATA + highest nChainWork per height), then fill `chain[h]` from buckets. O(M) time, O(tip_h) temp memory (24 MB at current depth). Optional secondary: reconnect anchor's `pprev` via `block_map_find(&ms->map_block_index, &anchor_header.hashPrevBlock)` before calling rebuild, so the pprev-walk completes in O(tip_h) on the happy path. **RED test:** wall-clock <2s on a synthetic N=100,000-depth block_map with `pprev=NULL` tip. | `app/services/src/chain_restore_service.c:322-391` (`chain_restore_rebuild_active_chain`) | **CRITICAL** | Agent-2 (chain_restore lane) — **NOW** |
