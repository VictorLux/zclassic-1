...
static bool value_overflow_hole_below_cursor_unlocked(sqlite3 *db, int cursor,
                                                      int *out_height)
{
    *out_height = -1;
    if (cursor <= 0)
    ...
    if (sqlite3_prepare_v2(db,
            ...
            -1, &st, NULL) != SQLITE_OK) {
        LOG_WARN("stage_repair",
                 "[stage_repair] value_overflow hole prepare failed: %s",
                 sqlite3_errmsg(db));
        return false;
    }
    ...
    if (rc == SQLITE_ROW) {
        *out_height = sqlite3_column_int(st, 0);
        ...
        LOG_WARN("stage_repair",
                 "[stage_repair] value_overflow hole step failed rc=%d: %s",
                                      ...
                                      struct uint256 *block_hash)
{
    ...
    if (uint256_cmp(&got, block_hash) != 0) {
        ...
        LOG_WARN("stage_repair",
                 "[stage_repair] value_overflow repair read wrong block "
                 "h=%d want=%s got=%s",
...
static bool maybe_repair_value_overflow_coin_tear(
    sqlite3 *db,
    struct main_state *ms,
    bool apply,
    struct stage_reducer_frontier_reconcile_result *out)
{
    ...
    bool ok = stage_repair_cursor_at_unlocked(db, "utxo_apply", &cursor) &&
              value_overflow_hole_below_cursor_unlocked(db, cursor, &height);
    progress_store_tx_unlock();
    if (!ok)
        return false;
    ...
    struct stage_reducer_frontier_reconcile_result *out)
{
    ...
    if (local.refused_coin_tear) {
        if (!maybe_repair_value_overflow_coin_tear(db, ms, apply, &local))
            return false;
        if (local.value_overflow_repaired) {
            LOG_WARN("stage_repair",
                     "[stage_repair] reducer_frontier repaired stale "
                     "value_overflow hole h=%d utxo_apply=%d->%d; "
                     "forward stage replay must fill the hole before L1 "
                     "continues",
                     local.value_overflow_repair_height,
                     local.value_overflow_cursor_before,
                     local.value_overflow_cursor_after);
            if (out)
                *out = local;
            return true;
        }
