 ...
 *
 * local_chain_ingest_fastimport — the three-phase fastimport pipeline
 * quarantined out of local_chain_ingest.c.
 *
 * This file owns:
 *
 *   Phase 1 (SHA3 window verify) — mmap'd blk*.dat scan against the
 *           compile-time g_sha3_windows[] table, with a persisted
 *           evidence cookie so subsequent boots skip an unchanged scan.
 *
 *   Phase 2 (chainstate import) — LevelDB read of the legacy
 *           <datadir>/chainstate/ via chainstate_legacy_reader, bulk-
 *           written to coins.db at the static SHA3 anchor height. A
 *           sidecar fast path and a persisted UTXO-SHA3 fingerprint can
 *           skip the work when the previous run's commitment is intact.
 *
 *   Phase 3 (per-block ingest) — header probe + legacy_body_pull over
 *           loopback JSON-RPC, then chain_advance() per block from the
 *           anchor up to the configured/legacy tip.
 *
 *   local_chain_ingest_run — the public orchestrator that runs the
 *           three phases in order. This is the entry point that
 *           -fastimport=PATH wires up.
 *
 * Fastimport is being demoted to advanced / per-block diagnostic in
 * cold-start consolidation (WS-1) — quarantining it here makes that
 * demotion mechanical: the rest of the module (shared state, accessors,
 * zcl_state dump) stays untouched.
 *
 * All shared progress state lives in g_local_ingest_state (declared in
 * local_chain_ingest_internal.h, defined in local_chain_ingest.c).
 */
...
/* Shorthand alias to keep call sites identical to the pre-split file. */
#define g_state g_local_ingest_state
    ...
    if (!all_ok) {
        for (int64_t w = 0; w < verifiable_windows; w++) {
            if (atomic_load(&tasks[w].mismatch)) {
                lci_state_set_error("phase1: window hash mismatch (parallel)");
                fprintf(stderr, // obs-ok:pre-existing-diagnostic
        ...
        if (r == LCI_OK) {
            ...
            lci_state_set_error("phase1: parallel verifier I/O failure");
        }
    }
    free(tasks);
    free(items);
    ...
    const char *our_datadir)
{
    ...
    if (num_files == 0) {
        lci_state_set_error("phase1: no blk*.dat files found");
    ...
    for (int f = 0; f < num_files; f++) {
        ...
        FILE *fp = fopen(path, "rb");
        if (!fp) {
            lci_state_set_error("phase1: open blk file failed");
            LOG_RETURN(LCI_SOURCE_MISSING, "local_ingest",
        ...
        for (;;) {
            ...
            if (!payload) {
                fclose(fp);
                lci_state_set_error("phase1: payload malloc failed");
            ...
            if (block_index_in_window == SHA3_WINDOW_SIZE) {
                ...
                if (current_window < (int64_t)g_sha3_windows_count) {
                    if (memcmp(digest,
                        ...
                        fclose(fp);
                        lci_state_set_error("phase1: window hash mismatch");
                            ...
                            void *vctx)
{
    ...
    if (!e) {
        lci_state_set_error("phase2: coins_view_cache_modify_new failed");
        return false;
    }
    ...
    if (!nv) {
        lci_state_set_error("phase2: vout calloc failed");
        return false;
    }
...
{
    if (!ms || !anchor_hash) {
        lci_state_set_error("phase2: missing CSR anchor context");
        LOG_RETURN(LCI_INTERNAL_ERROR, "local_ingest",
    ...
    if (!anchor_bi || !anchor_bi->phashBlock) {
        lci_state_set_error("phase2: verified anchor missing from block index");
        LOG_RETURN(LCI_INTERNAL_ERROR, "local_ingest",
                   "phase2: verified anchor h=%d missing from block index",
                   anchor_height);
    }
    if (anchor_bi->nHeight != anchor_height) {
        lci_state_set_error("phase2: verified anchor height mismatch");
        LOG_RETURN(LCI_INTERNAL_ERROR, "local_ingest",
    ...
    const char *our_datadir)
{
    ...
    if (!coins_tip) {
        lci_state_set_error("phase2: NULL coins_tip");
        LOG_RETURN(LCI_INTERNAL_ERROR, "local_ingest",
                   "phase2: coins_tip is NULL");
    }
    if (!ms) {
        lci_state_set_error("phase2: NULL main_state");
        LOG_RETURN(LCI_INTERNAL_ERROR, "local_ingest",
        ...
        if (stat(uss_path, &ust) == 0 && S_ISREG(ust.st_mode)) {
            ...
            if (anchor_pre) {
                ...
                if (uh) {
                    ...
                    if (!cvs2 || !cvs2->db) {
                        uss_close(uh);
                        lci_state_set_error("phase2 fast-path: "
                    ...
                    if (!batch) {
                        uss_close(uh);
                        lci_state_set_error("phase2 fast-path: oom batch");
                        LOG_RETURN(LCI_INTERNAL_ERROR, "local_ingest",
                    ...
                    if (errors || (uint64_t)inserted != hdr.count) {
                        lci_state_set_error("phase2 fast-path: bulk insert "
                                        "errors");
                        LOG_RETURN(LCI_INTERNAL_ERROR, "local_ingest",
    ...
    if (stat(cs_path, &st) != 0 || !S_ISDIR(st.st_mode)) {
        lci_state_set_error("phase2: chainstate dir missing");
        LOG_RETURN(LCI_SOURCE_MISSING, "local_ingest",
    ...
    if (!chainstate_legacy_open(cs_path, &h) || !h) {
        lci_state_set_error("phase2: chainstate_legacy_open failed");
        LOG_RETURN(LCI_SOURCE_MISSING, "local_ingest",
    ...
    if (!anchor) {
        ...
        lci_state_set_error("phase2: no SHA3 anchor available");
        LOG_RETURN(LCI_INTERNAL_ERROR, "local_ingest",
    ...
    chainstate_legacy_close(h);
    if (n < 0) {
        lci_state_set_error("phase2: chainstate iter failed");
        LOG_RETURN(LCI_INTERNAL_ERROR, "local_ingest",
    ...
    if (!coins_view_cache_flush(coins_tip)) {
        lci_state_set_error("phase2: post-import cache flush failed");
        LOG_RETURN(LCI_INTERNAL_ERROR, "local_ingest",
    ...
    if (!cvs || !cvs->db) {
        lci_state_set_error("phase2: coins_view_sqlite unavailable");
        LOG_RETURN(LCI_INTERNAL_ERROR, "local_ingest",
    ...
    if (!shape_matches_anchor) {
        lci_state_set_error("phase2: imported chainstate shape does not match anchor");
    ...
    if (memcmp(computed_sha3, anchor->sha3_hash, 32) == 0) {
        ...
        lci_state_set_error("phase2: SHA3 anchor mismatch");
        LOG_RETURN(LCI_CHAINSTATE_MISMATCH, "local_ingest",
    ...
    const char *our_datadir)
{
    ...
    if (!ms || !params || !our_datadir) {
        lci_state_set_error("phase3: missing context");
        LOG_RETURN(LCI_INTERNAL_ERROR, "local_ingest",
    ...
    const char *our_datadir)
{
    ...
    if (!local_chain_ingest_detect_legacy_datadir(cfg->legacy_datadir)) {
        lci_state_set_datadir(cfg->legacy_datadir);
        lci_state_set_error("legacy datadir missing blocks/blk00000.dat");
