 ...
 *
 * Cutover preflight — the read-only `cutoverpreflight` RPC: a conservative
 * "ready" snapshot across every gate the guarded Wave-S authority flip must
 * pass before authority may move onto the log-derived pipeline.
 *
 * Split out of cutover_controller.c (which still owns the cutovermode RPC
 * that ACTS on this snapshot) to keep each file under the app/ file-size
 * ceiling. The two files share cutover_push_canary_state() via
 * controllers/diagnostics_internal.h.
 *
 * The gates ANDed into `ready` are intentionally conservative: every gate
 * can only make `ready` HARDER to achieve, never easier. A refused flip
 * lists a typed blocker per failing gate so the refusal explains itself —
 * never a silent false. Among the gates is the shadow-pipeline conservation
 * law (fed == diffed): authority must never flip onto a pipeline that has
 * been silently dropping blocks. See push_cutover_conservation_gate_json()
 * for the quiesced-vs-in-flight handling (errs toward not-ready).
 */
...
/* Mode-name helpers. Trivial pure mappers; duplicated here (rather than
 * shared) so this file does not depend on cutover_controller.c internals. */
...
/* Shadow-pipeline conservation gate. The law (see
 * adapters/inbound/shadow_conservation.h) is `fed == diffed` at quiesce:
 * every block FED into the shadow pipeline was actually DIFFED against
 * legacy. If the pipeline silently dropped a block, fed > diffed — and we
 * must NOT flip authority onto a pipeline that has been dropping blocks.
 *
 * QUIESCED-VS-IN-FLIGHT: the diff lags the live feeder, so a transient
 * fed > diffed while blocks are in flight is EXPECTED and is not a true
 * violation. This preflight is a CONSERVATIVE ready snapshot — it gates on
 * the strict snapshot equality, so an in-flight delta makes the gate read
 * "not ready" and simply DEFERS the flip until the diff catches up. That is
 * the SAFE direction (erring toward not-ready); it never weakens the gate.
 * The other live gates (tip_lag == 0, tip_recent, cursor_lag == 0) already
 * require an effectively-quiesced/caught-up pipeline, so a residual fed !=
 * diffed at that point is a genuine drop. Best-effort observe-only counters;
 * reading them never gates the hot path. */
static bool push_cutover_conservation_gate_json(struct json_value *out,
                                                 unsigned long *fed_out,
                                                 unsigned long *diffed_out)
{
    unsigned long fed = 0, diffed = 0, skipped = 0;
    bool conserved = shadow_conservation_ok(&fed, &diffed, &skipped);
    if (fed_out) *fed_out = fed;
    if (diffed_out) *diffed_out = diffed;

    json_set_object(out);
    json_push_kv_bool(out, "ok", conserved);
    json_push_kv_int(out, "fed", (int64_t)fed);
    json_push_kv_int(out, "diffed", (int64_t)diffed);
    json_push_kv_int(out, "skipped", (int64_t)skipped);
    json_push_kv_str(out, "law", "fed == diffed (every fed block diffed)");
    return conserved;
}
                               ...
                               struct json_value *result)
{
    ...
    cutover_push_canary_state(&canary, &live_health);
    bool guard_ready = push_cutover_guard_gate_json(&guard);
    unsigned long cons_fed = 0, cons_diffed = 0;
    ...
    if (!conservation_ok) {
        /* Name the drop so a refused flip EXPLAINS itself (no silent
         * false): which side is short and by how much. */
        char reason[96];
        snprintf(reason, sizeof reason,
                 "shadow_pipeline_dropped_blocks: fed=%lu diffed=%lu",
                 cons_fed, cons_diffed);
