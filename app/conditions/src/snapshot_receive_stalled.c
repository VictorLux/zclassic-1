...
#endif
    fprintf(stderr,  // obs-ok:condition-snapshot-receive-stalled
            "[condition:snapshot_receive_stalled] peer=%u "
            "elapsed=%llds received=%llu offered=%llu action=reset\n",
            (unsigned)atomic_load(&g_peer_at_detect),
            (long long)atomic_load(&g_elapsed_at_detect),
            (unsigned long long)atomic_load(&g_received_at_detect),
            (unsigned long long)atomic_load(&g_offered_at_detect));
...
{
    struct condition_state *s = &c_snapshot_receive_stalled.state;
