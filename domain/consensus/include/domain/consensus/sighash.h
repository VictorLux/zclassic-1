 ...
 * with n_in >= num_vout in the Sprout regime). On failure a diagnostic
 * is written to stderr in the domain "sighash" via LOG_FAIL — the
 * domain layer remains pure (no allocator, no I/O state, no clock).
 *
