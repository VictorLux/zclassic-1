...
> **⇒ Active plan: [`WAVE_12.md`](./WAVE_12.md).** Wave 11 COMPLETE (13/13). Wave 12 theme: battle-tested under adversarial conditions. Priority: (1) MCP input fuzzing, (2) RPC auth hardening, (3) memory leak audit, (4) wallet backup/HD integration tests, (5) OpenAPI spec. Prove everything works under stress.
...
- **2026-04-12 wave 10 session 3** — **#3 done.** peer_bandwidth wired into `connman.c` socket handler loop. Download: `peer_bandwidth_available()` gate before `recv()`, recv size capped to available tokens, `peer_bandwidth_consume()` post-recv. Upload: available check before `socket_send_data()`, track actual bytes via `node->send_bytes` delta, consume post-send. Throttled peers skipped for that 50ms poll cycle — no disconnect, resume on refill. Global `struct peer_bandwidth` initialized from env at `connman_start()` (ZCL_PEER_UP_BPS/DOWN_BPS/BURST), destroyed at `connman_stop()`. `connman_peer_bandwidth()` getter in `connman.h` for metrics. Defaults: 10 MB/s up, 20 MB/s down, 1 MB burst. `ZCL_PEER_UP_BPS=0` disables. `./test_zcl`: **ALL TESTS PASSED (0 failures)**. **Next up**: #4 RPC cookie rotation, #5 Sapling key scrubbing, #6 dep vulnerability scan.
