...
- **2026-04-11 wave 4 session 4** — **#2 done.** HTTP RPC middleware. New `lib/rpc/include/rpc/http_middleware.h` + `lib/rpc/src/http_middleware.c` providing global token bucket (50 rps / 100 burst) + per-IP token bucket (5 rps / 10 burst, LRU table capped at 256) + IP ban after 5 auth failures (default 1h ban) — all env-tunable via `ZCL_RPC_RPS`, `ZCL_RPC_BURST`, `ZCL_RPC_PER_IP_RPS`, `ZCL_RPC_PER_IP_BURST`, `ZCL_RPC_AUTH_FAIL_THRESHOLD`, `ZCL_RPC_BAN_SECONDS`. Loopback (127.0.0.0/8) bypasses ban + per-IP buckets but still counts against the global bucket so a buggy local script can't DoS the server. Wired into `lib/rpc/src/httpserver.c::handle_client()`: pre-flight check returns HTTP 403 (banned) or 429 (rate limited) before parsing the request; auth failures and successes feed back into the per-IP bad-actor counter. Initialised in `rpc_http_start()`, torn down in `rpc_http_stop()`. 12 new tests in `lib/test/src/test_http_middleware.c` cover defaults, loopback bypass, per-IP cap, global cap, ban-on-fifth-auth-failure, success-resets-counter, env overrides, env-tuned threshold, disable-via-env, LRU eviction, reset_state, and stat counters.
- **2026-04-11 wave 4 session 5** — **#3 done (primitives).** Wallet key encryption at rest — passphrase-based AES-256-GCM via OpenSSL EVP. New `lib/wallet/include/wallet/wallet_keystore.h` + `lib/wallet/src/wallet_keystore.c`: `wks_encrypt()` derives a 256-bit key via PBKDF2-HMAC-SHA512 (200_000 default iterations, env-tunable via `ZCL_WALLET_KDF_ITERS`, clamped to [10k, 10M]) from a per-call random salt, then encrypts under AES-256-GCM with a fresh 12-byte nonce and 16-byte auth tag. Envelope format: 4-byte `WKS1` magic + version + iters + reserved + 16B salt + 12B nonce + 16B tag + ciphertext (60B header total). `wks_decrypt()` validates magic/version, derives the key from the stored salt, and verifies the GCM tag — wrong passphrase or any single bit flipped in the ciphertext both fail closed. **Live wallet integration is a follow-up**: this commit ships the primitives and the test coverage; wiring through wallet_db.c / wallet.c / keystore.c / wallet_key.c / wallet_sqlite.c (~3500 lines) needs its own session with regression coverage on every controller that touches a key, and that risk doesn't belong in the same commit as the primitives. 14 unit tests in `lib/test/src/test_wallet_keystore.c` cover round-trip, wrong passphrase rejected, tampered ciphertext rejected, tampered tag rejected, unique-envelopes-per-call (random salt+nonce), header magic + iterations, undersized output buffer rejected, iteration clamping, env-tuned default iters, truncated envelope rejected, bad magic rejected, zero-length plaintext, 1024-byte plaintext, NULL passphrase rejected. Also caught and fixed an include-guard collision: legacy `lib/wallet/include/wallet/keystore.h` already used `ZCL_WALLET_KEYSTORE_H`, so the new header uses `ZCL_WALLET_KEYSTORE_AT_REST_H`.
