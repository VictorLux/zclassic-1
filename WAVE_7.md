...
- [x] **PHGR13 sync stall investigation** — `PHGR13_INVESTIGATION.md` landed. Primary hypothesis: `ppzksnark_vk_read` (`lib/sapling/src/bn254.c:1687`) assumes a flat-concat-of-BE-Fq layout, but `sprout-verifying.key` is libsnark's native `operator<<` binary (Montgomery limbs + OUTPUT_NEWLINE + nested `accumulation_vector<G1>`). Evidence: at offset 768 the "ic_len u32" reads as 516,817,727 → sanity check fails → `phgr_vk` stays NULL → every `sprout_verify_phgr13` short-circuits on `!phgr_vk`. Secondary bug: `bn_g2_decompress` reads compressed G2 x as `concat(c1, c0)` instead of Zcash FE2IP (`c1·q + c0` as 512-bit BE, see `zclassic-cpp/src/zcash/Proof.cpp:85-95`). Both must ship together. Fix sketch + test plan + ruled-out branches in the doc. Not committing the fix per the plan.
...
- [x] **IBD throttle** — `app/services/ibd_throttle.{h,c}` landed in `a06f8ded1` (before wave 7 refresh). Token-bucket with env tunables `ZCL_IBD_BLOCKS_PER_SEC` (default 500) + `ZCL_IBD_BURST` (default 50), blocking `acquire()` + non-blocking `try_acquire()`, lock-free `ibd_throttle_refill` pure primitive for tests, rate-limited `EV_IBD_THROTTLED` (at most once/60s with aggregated `blocked/total_wait/rate/burst`). 36 tests. Hot-path wiring in `process_block.c` queued in `BOOT_QUEUE.md`.
