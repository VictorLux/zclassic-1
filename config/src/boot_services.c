 ...
 * The background_utxo_replay() thread (just below) reads
 * coins_best_block and CSR-commits the chain tip to the snapshot
 * height; activate_best_chain then advances from there rather than
 * from genesis.
 *
...
static bool boot_import_snapshot_db(struct node_db *ndb,
