...
| ✅ | `lib/sapling/src/bn254.c` (`ppzksnark_verify` G2 generator) | **was a CONSENSUS bug, not just noise:** the hardcoded alt_bn128 G2 generator constants were CORRUPTED (off-curve), so `g2_gen`/`G2_one` was garbage and the verifier false-rejected EVERY Sprout PHGR13 proof at pairing check 1 — flooding node.log (92%/133% CPU) AND meaning the node could not re-verify its own pre-Sapling history (same verifier is on the consensus path). Found by multi-agent audit (`6f23272c8`). | **DONE:** replaced constants with the canonical generator (independently verified on-curve); single-source `bn254_g2_one()` shared with tests (no more drift); on-curve guard `bn_g2_is_on_curve()` makes future corruption LOUD; regression test runs unconditionally. **Live-validated:** post-deploy bg-validation verified 1,687 proofs with 0 rejections. | done |
