...
- [~] **C1** `chain_advance_coordinator.c` (1,355) → pure source-selection policy extracted to `block_source_policy.{h,c}` behind CAC compatibility wrappers; mirror force-promotion window **deleted** (watchdog/supervisor now emit named stall/revalidation-exhausted events instead of forcing mirror). Remaining: rewire surviving consumers/status/dump surface and shrink/delete the CAC shell. *Gated on B.*
...
- [~] **D5** `app/` files > 800 LOC: **31 → 5** under the cap (E1 baseline file: `tools/scripts/file_size_ceiling_baseline.txt`). Wave splits landed: 12 oversized controllers (`ec1424cf4`), 3 views (`7e52a3c14`), explorer_factoids (`f3e57804d`), `database.c`+`wallet_tx.c` models (`c945d64c4`), api+snapshot router monoliths (`adc7ac45a`), `validate_headers` + `bg_validation` (`46d0edb2a`), reconciled (`7ca847c57`), C1 policy extraction, C3 `utxo_recovery_service.c` 1141→624 split. **Remaining 5 are all legacy modules B8 deletes/shrinks**: `chain_advance_coordinator.c` (1355), `legacy_mirror_sync_service.c` (1180), `sync_controller_catchup.c` (1262), `legacy_import.c` (1105), `sync_controller_import.c` (995). Closing D5 = finishing the cutover (C+B8), not more splits.
...
- [x] **E1** `file-size-ceiling` — RATCHET, baseline = **5** grandfathered files (down from 29) (`tools/scripts/file_size_ceiling_baseline.txt`). All 5 remaining are legacy modules B8 deletes. `5daf21742`.
- [x] **E2** `check-one-result-type` — RATCHET (**6** service files baselined, down from 77; new ones must use `zcl_result`) (`tools/scripts/one_result_type_baseline.txt`; `grep -vc '^#\|^$'` → 6). `f331f6e0d` + A4 paydowns through this round.
...
- [~] **E6** `one-write-path` — RATCHET wired (`tools/scripts/check_one_write_path.sh`), baseline = **74** legacy write surfaces; hardens as B8 deletes them.
