...
- [x] **Ports/adapters seam populated (2026-05-27):** `ports/include/ports/` has **9** port headers; **4 services** now read/write through a port instead of touching SQLite directly — hodl_history (`hodl_history_port.h`), node_health (`node_health_store_port.h`), db_maintenance (`db_maintenance_port.h`), wallet_backup (`wallet_backup_store_port.h`) — plus the pre-existing block_log, clock, consensus_log, event_emitter, utxo_snapshot ports. SQLite adapters live in `adapters/outbound/persistence/src/` (`hodl_history_sqlite.c`, `node_health_store_sqlite.c`, `db_maintenance_sqlite.c`, `wallet_backup_store_sqlite.c`, …), each with a port test (`lib/test/src/test_*_port.c`). `block_index_sidecar` is NOT yet a port (verify: `ls ports/include/ports/ | grep -i sidecar` → none). Most storage still calls `lib/storage/*_sqlite.c` directly; this is a real seam, not full coverage.
...
- [~] **B5** Make `log_head` / the `tip_finalize` cursor the **definitional tip**. Demote `chain_active` to a derived in-RAM index rebuilt from `block_index_projection`. *Acceptance:* `health = network_tip − log_head` is one real number.
  ...
  - **Flip is proven cheap** ([`work/b5-chain-active-readers.md`](./work/b5-chain-active-readers.md)): readers already route through two accessors `active_chain_tip(c)` / `active_chain_height(c)` (`lib/validation/include/validation/chainstate.h`), so the flip is a 2-accessor-body change, not a chase across ~20 ad-hoc call sites. The audit table covers ~133 reader rows, almost all already through the accessor. **Still PENDING — this step does NOT flip authority, it only confirmed the seam.**
...
- [ ] **B7** Flip once, behind the guard. `cutovermode all authoritative` + `cutover_no_forward_progress` auto-revert (180s no-progress → revert to SHADOW + page). Real canary: one block connects through the authoritative path, auto-revert on any divergence. **NOT done.** *The safety net it needs already exists and is tested:* `test_cutover_tip_parity.c` (log_head tip == chain_active per height, incl. reorg), `test_reorg_parity.c` (path-independent UTXO-commitment recompute), `test_shadow_conservation.c` (fed==diffed), `test_replay_verify.c` (PoW-in-CI), and the runtime preflight conservation gate `app/controllers/src/cutover_controller_preflight.c` (grep `shadow_pipeline_dropped_blocks`). What remains is the flip itself + arming the canary on a clean datadir.
- [ ] **B8** Extract-then-delete (see **C**) — the legacy path + the entire shadow-vs-legacy comparison apparatus (`diff_with_legacy_shadow`, `shadow_feeder`, the `*_projection_diff` MCP tools, cutover mode/preflight/canary plumbing). Once there's one path, the comparison apparatus is dead. **NOT done.** Inventory corrected in [`work/b8-deletion-inventory.md`](./work/b8-deletion-inventory.md): net deletable ≈ **3,900–4,000 LOC** (importer ~1,721 + CAC net-of-moved-policy ~1,100 + mirror block-apply ~600 + body_pull 529), NOT the old 4,407/6,059 headlines. `connect_tip` SURVIVES (it is the post-flip authoritative connect path, the cutover *destination*), `legacy_import` controller is KEEP, `utxo_recovery` is mostly KEEP/A-work, and the `legacy_mirror` heartbeat + lag-SLO monitor are PRESERVED (only its block-application half is deleted).
...
- [~] **A4** `zcl_result` adoption for services (Law 2). Migrate services off bare `bool`/`int`. Cluster-by-cluster in flight: wallet-backup (`844f13472`), snapshot (`20bb5bc36`), ZSLP (`c2e107257`), infra (`93ac12748`), chain_restore (`922ea6fff`), chain_tip (`655804a9b`), oracle+policy (`36bb634da`). E2 baseline ratcheted 77 → **33** (`tools/scripts/one_result_type_baseline.txt`; `grep -vc '^#\|^$'` → 33). *(ratchet — see E2)*
...
- [x] **E2** `check-one-result-type` — RATCHET (**33** service files baselined, down from 77; new ones must use `zcl_result`) (`tools/scripts/one_result_type_baseline.txt`; `grep -vc '^#\|^$'` → 33). `f331f6e0d` + A4 paydowns through `e7e8895ee`.
...
- **2026-05-27** Recovery moles closed and pinned: BIP30 self-write tolerance + CEC cursor non-gating + LOCAL_IMPORT reconstruct + stale-freeze reconcile + E8 no-silent-ready. The next move is **B5 → B7** on a clean datadir (canary armed). Doc reconciled against live lint baselines: E1 file-size 31→7, E2 one-result-type 77→33; A5 codegen shipped; B4 read-mechanism + authority-gated wiring DORMANT.
- **2026-05-27** Canon reconciled with reality (docs-only): `domain/` now holds 21 pure modules (3 contexts) each with a legacy wrapper + a seal test; ports/adapters seam populated for 4 services (9 port headers total; `block_index_sidecar` not yet a port); E2 baseline 77→33; B7 safety net (tip-parity / reorg-parity / shadow-conservation / replay-verify tests + preflight conservation gate) exists but the flip is NOT done; B8 net deletable corrected 4,407→≈3,900–4,000 (`connect_tip` survives, `legacy_mirror` heartbeat preserved); B5 proven a 2-accessor-body change. B7/B8 remain open.
