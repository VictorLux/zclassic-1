...
- [ ] **B4** Point `connect_block` input lookups at the utxo **projection** instead of `coins_view_cache`. Closes the validation feedback loop (Prime Directive). Keep a RAM read-cache for speed; authority is the projection; its write lands in the same txn as the cursor advance.
  ...
  - **PREREQUISITE found:** `struct coins.version` is plumbed into undo (`update_coins.c:154`) and restored on disconnect (`connect_block.c:709`); the projection `utxo` table + `EV_UTXO_ADD` payload do **not** carry `version`. A faithful adapter must round-trip it. So B4 first adds a `version` column end-to-end: `ev_utxo_add_hdr` payload (+serialize/parse), projection schema (migration), **both** emitters (`update_coins_emit_utxo_add_shadow` and `utxo_apply_stage`'s `delta_entry`/`emit_delta`), and the `utxo_projection_commitment` serialization (keep it matching `utxo_commitment.c`). Prove via `test_utxo_apply_authorship` (extend) + the B6 driver that commitment stays byte-exact with `version` included. This is a coordinated consensus change — do it as one focused effort, not a mid-merge slice.
