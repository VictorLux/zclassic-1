...
1. ⚡ [`wt-snapshot-wedge-recovery.md`](./work/wt-snapshot-wedge-recovery.md) **PR-1: arm snapshot recovery on a wedged tip** — today fast-sync is cold-start-only, so a node wedged at the tip (`block_failed_mask_at_tip` exhausted, `next-child-missing`) just sits there. Add a `tip_wedged_resnapshot` Condition that re-snapshots *past* the bad block (full FlyClient+SHA3, no trust shortcut). Moves #6 recovery + #5 keep-up. **Take this — observed wedging a live node right now.**
2. [`wt-perf-integrate-rebuild.md`](./work/wt-perf-integrate-rebuild.md) PR-2/PR-3 — PR-1 (hardware CRC32C in event_log.c) ✅ shipped `69939ec97`. Remaining: io_uring bulk-append (PR-2), parallel segmented cold-import (PR-3). Moves #1/#2.
