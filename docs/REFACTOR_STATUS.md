...
- [~] **B4** Point `connect_block` input lookups at the utxo **projection** instead of `coins_view_cache`. Closes the validation feedback loop (Prime Directive). Keep a RAM read-cache for speed; authority is the projection; its write lands in the same txn as the cursor advance.
  ...
  - **DONE (B4-wiring):** the authority-gated backing selection at connect time. New `lib/storage/coins_view_stage_backing.c` — `coins_view_select_connect_backing()` chooses the `coins_view_cache` backing on `utxo_projection_get_author()`: LEGACY (default) hands back the coins_tip view *verbatim* (byte-identical to today); STAGE returns a composite that resolves get_coins/have_coins through the projection (authoritative reads) while delegating get_best_block + batch_write to the coins_tip view (best-block consistency for the prevblock invariant; batch_write keeps the coins.db mirror warm and is NOT a second projection writer — B3 single-writer holds because the projection is authored by events). Wired at the one seam `connect_tip.c` (`coins_view_cache_as_view` → select → `coins_view_cache_init`); connect_block call sites byte-identical; RAM read-cache preserved. Proven in `test_coins_view_projection` (extended): under STAGE, cache reads of the same set are byte-identical to the coins.db-backed reads (value/script/null-vout/best-block), and LEGACY/re-LEGACY return the legacy view's exact vtable+impl; STAGE-without-projection falls back to legacy (never crashes). DORMANT until B7 (default author LEGACY → live path unchanged).
