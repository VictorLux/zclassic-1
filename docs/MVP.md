...
| 7 | **Recover from `kill -9` in <2 min** | kill -9 mid-block, restart, caught up to peer-tip within 2 min — in-process unit `lib/test/src/test_kill9_recovery.c` (**CI slice: `make ci-mvp-gates`** — proves `node.db` atomic UTXO recovery after SIGKILL) **plus** the full-binary harness `make test-crash-bootstrap` (opt-in, NOT in `make ci`) which spawns a real isolated regtest node and proves clean **boot recovery** under a real process-group `SIGKILL`. Still ◐: on this build regtest `generate` mines no blocks — fully root-caused 2026-06-05 (layered; see [`work/regtest-ondemand-mining-rootcause.md`](./work/regtest-ondemand-mining-rootcause.md)): L1 = the mined block never reaches the block_index (`reducer_ingest_block` pushes the carried header with `height=-1`, which `handle_header_admit_msg` drops before staging it — fix touches the live shared header-admission path, owner-gated/prove-on-copy); L2 = `tip_finalize`'s one-block lookahead can't finalize a successor-less self-mined tip (vetted regtest-gated `set_authoritative_tip` fix, dead until L1). So the zero-UTXO-above-tip overshoot teeth report `over=-1` (not-applicable) pending those; and the literal restart-to-**peer**-tip needs the operational two-node `--with-peer` variant. | ◐ |
