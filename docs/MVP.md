...
| 7 | **Recover from `kill -9` in <2 min** | kill -9 mid-block, restart, caught up to peer-tip within 2 min — in-process unit `lib/test/src/test_kill9_recovery.c` (**CI slice: `make ci-mvp-gates`** — proves `node.db` atomic UTXO recovery after SIGKILL) **plus** the full-binary harness `make test-crash-bootstrap` (opt-in, NOT in `make ci`) which spawns a real isolated regtest node and proves clean **boot recovery** under a real process-group `SIGKILL`. Still ◐: on this build regtest `generate` mines no blocks — root-caused 2026-06-05: a self-mined block passes PoW/`check_block` but `reducer_ingest_block` rejects it `block-not-finalized-by-reducer` because `tip_finalize` needs a one-block lookahead (a successor header in the active chain) that on-demand mining cannot produce, so the tip never advances off genesis (each `create_new_block` rebuilds the same block) — so the zero-UTXO-above-tip overshoot teeth report `over=-1` (not-applicable) pending the regtest instant-finalize fix; and the literal restart-to-**peer**-tip needs the operational two-node `--with-peer` variant. | ◐ |
