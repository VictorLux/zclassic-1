...
- [ ] **Addrman persistence robustness** — `lib/net/src/addrman.c` writes `peers.dat` but has no checksum, no atomic rename, no corruption detection. Add SHA3 sidecar + atomic rename (write to `.tmp`, fsync, rename). Refuse to load on checksum mismatch. Same pattern as `block_index_integrity`.
