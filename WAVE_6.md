...
- [x] **#2 PHGR13 sync stall investigation** — `PHGR13_INVESTIGATION.md` landed. Primary hypothesis: `ppzksnark_vk_read` in `lib/sapling/src/bn254.c:1687` assumes a flat-concat-of-BE-Fq layout for `sprout-verifying.key`, but libsnark's actual serializer uses `operator<<` with OUTPUT_NEWLINE separators, Montgomery-form limbs in native LE order, and a nested `accumulation_vector<G1>` for `encoded_IC_query`. Evidence: at offset 768 the "ic_len u32" decodes to 516,817,727, failing the sanity check → `phgr_vk` stays NULL → every `sprout_verify_phgr13` short-circuits on `!phgr_vk`. Secondary bug: `bn_g2_decompress` reads compressed G2 x as `concat(c1, c0)` instead of Zcash's FE2IP (`c1·q + c0` as 512-bit BE). Both must ship together. Fix sketch + test plan in the doc; not committing the fix in this session per the plan.
...
- [x] **#1 RPC timeout layer** — `lib/rpc/{include/rpc,src}/rpc_timeout.{h,c}` with 128-slot in-flight table, per-request register/set_method/unregister, synchronous `rpc_timeout_sweep(now_us)` (for unit tests) + background watchdog pthread that runs sweep every `ZCL_RPC_TIMEOUT_SWEEP_MS` (default 250, clamped [10, 60000]). Deadline from `ZCL_RPC_TIMEOUT_MS` (default 10000; `0` disables entirely). Sweep marks the slot killed, `shutdown(fd, SHUT_RDWR)`s the socket so the worker's in-flight read/write unwinds, then emits one `EV_RPC_TIMEOUT` per kill with `method=NAME elapsed_ms=N ip=A.B.C.D`. Wired into `httpserver.c`: register at top of `handle_client()`, `set_method()` after JSON parse, `unregister()` at `done:`. New `EV_RPC_TIMEOUT` event type (`rpc.timeout`) added to `lib/event/{include/event/event.h,src/event.c}`. 14 unit tests in `test_rpc_timeout.c` cover init defaults, register/unregister roundtrip, method truncation, table-full, disabled module no-op, sweep-no-expiries, sweep-kill + event payload shape, empty-method fallback, re-sweep idempotence, env overrides + clamping, snapshot consistency, global handle set/get, **live watchdog thread** (socketpair fd, 50ms deadline, polls `was_killed`), and reset_state.
